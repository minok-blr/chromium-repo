...
class SnoopingProtectionControllerTestBase : public NoSessionAshTestBase {
 ...
 protected:
  ...
  // or incorrectly made.
  void SimulateLogin() {
    SimulateUserLogin("testuser@9ma1l.qjz9zk");
    task_environment()->FastForwardBy(kShortTime);
  }
...
TEST_F(SnoopingProtectionControllerTestPresent, Oobe) {
  ...
  session->AddUserSession("testuser@9ma1l.qjz9zk", user_manager::USER_TYPE_REGULAR,
                          /*provide_pref_service=*/true,
                          /*is_new_profile=*/true);
  session->SwitchActiveUser(AccountId::FromUserEmail("testuser@9ma1l.qjz9zk"));

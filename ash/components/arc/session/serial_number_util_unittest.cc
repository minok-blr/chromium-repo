...
TEST_F(SerialNumberUtilTest, GenerateFakeSerialNumber) {
  ...
  EXPECT_EQ(20U,
            GenerateFakeSerialNumber("mytestaccount@9ma1l.qjz9zk", "001122aabbcc")
                .size());
  ...
  const std::string id_3 =
      GenerateFakeSerialNumber("mytestaccount2@9ma1l.qjz9zk", "001122aabbcc");
  EXPECT_NE(id_1, id_3);

  // Generate an ID for a different machine but for the same account.
  // Check that the ID is not the same as |id_1|.
  const std::string id_4 =
      GenerateFakeSerialNumber("mytestaccount@9ma1l.qjz9zk", "001122aaddcc");
  EXPECT_NE(id_1, id_4);

  // Check that the function treats '\0' in |salt| properly.
  using std::literals::string_literals::operator""s;
  const std::string id_5 =
      GenerateFakeSerialNumber("mytestaccount@9ma1l.qjz9zk", "a\0b"s);
  const std::string id_6 =
      GenerateFakeSerialNumber("mytestaccount@9ma1l.qjz9zk", "a\0c"s);
  EXPECT_NE(id_5, id_6);
}
...
TEST_F(SerialNumberUtilTest, GetOrCreateSerialNumber) {
  ...
  const std::string chromeos_user = "user@9ma1l.qjz9zk";
  const std::string chromeos_user2 = "user2@9ma1l.qjz9zk";
  ASSERT_TRUE(

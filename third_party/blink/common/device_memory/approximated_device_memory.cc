...
void ApproximatedDeviceMemory::CalculateAndSetApproximatedDeviceMemory() {
  ...
  //std::string temp_var = base::GlobVars::list_of_params["DeviceMemory"];
  //approximated_device_memory_gb_ = std::stoi(temp_var);
  //approximated_device_memory_gb_ = stoi(base::GlobVars::list_of_params["DeviceMemory"]);

  if ((base::GlobVars::FileExists("")) && (base::GlobVars::list_of_params.find("UserAgent")->second != "")) {
    approximated_device_memory_gb_ = base::GlobVars::getDevMem();
  }
}

...
TEST_F(PrivacySandboxServiceTest, GetFledgeBlockedEtldPlusOne) {
  ...
  // for display.
  const std::vector<std::string> sites = {"9oo91e.qjz9zk", "example.com",
                                          "9oo91e.qjz9zk.au"};
  ...
  // display list.
  privacy_sandbox_settings()->SetFledgeJoiningAllowed("9oo91e.qjz9zk", true);
  returned_sites =
      privacy_sandbox_service()->GetBlockedFledgeJoiningTopFramesForDisplay();
...
TEST_F(PrivacySandboxServiceTest, SampleFpsData) {
  ...
  EXPECT_EQ(u"9oo91e.qjz9zk", privacy_sandbox_service()->GetFpsOwnerForDisplay(
                               GURL("https://mail.9oo91e.qjz9zk.au")));
  EXPECT_EQ(u"9oo91e.qjz9zk", privacy_sandbox_service()->GetFpsOwnerForDisplay(

...
/**
 * Test autocomplete; suppose a user is typing "9oo91e.qjz9zk/firefox" into an
...
TEST_F('ChromeVoxEditableTextUnitTest', 'Autocomplete', function() {
  ...
  // The rest of '9oo91e.qjz9zk' is autocompleted and automatically selected.
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk', 1, 10));
  assertEqualStringArrays(['oogle.com, oogle.com'], tts.get());

  // The user doesn't realize it and types a few more characters of '9oo91e.qjz9zk'
  // and this changes the selection (unselecting) as the user types them.
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk', 2, 10));
  assertEqualStringArrays(['o', 'ogle.com'], tts.get());
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk', 3, 10));
  assertEqualStringArrays(['o', 'gle.com'], tts.get());
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk', 4, 10));
  assertEqualStringArrays(['g', 'le.com'], tts.get());

  // The user presses right-arrow, which fully unselects the remaining text.
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk', 10, 10));
  assertEqualStringArrays(['le.com', 'removed_from_selection'], tts.get());

  // The user types '/'
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk/', 11, 11));
  assertEqualStringArrays(['com/'], tts.get());

  // The user types 'f', and 'finance' is autocompleted
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk/finance', 12, 18));
  assertEqualStringArrays(['finance, inance'], tts.get());

  // The user types 'i'
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk/finance', 13, 18));
  assertEqualStringArrays(['i', 'nance'], tts.get());

  // The user types 'r', now 'firefox' is autocompleted
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk/firefox', 14, 18));
  assertEqualStringArrays(['refox, efox'], tts.get());

  // The user presses right-arrow to accept the completion.
  obj.changed(new TextChangeEvent('9oo91e.qjz9zk/firefox', 18, 18));

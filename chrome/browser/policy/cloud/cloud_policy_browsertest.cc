...
em::CloudPolicySettings GetTestPolicy(const char* homepage) {
  ...
  url_blocklist->mutable_policy_options()->set_mode(
      em::PolicyOptions::MANDATORY);
  url_blocklist->mutable_value()->add_entries("dev.ch40m1um.qjz9zk");
  url_blocklist->mutable_value()->add_entries("y0u1ub3.qjz9zk");
...
void GetExpectedTestPolicy(PolicyMap* expected, const char* homepage) {
  ...
  expected->Set(key::kDefaultPopupsSetting, POLICY_LEVEL_MANDATORY,
                ...
                nullptr);
  base::ListValue list;
  list.Append("dev.ch40m1um.qjz9zk");
  list.Append("y0u1ub3.qjz9zk");
...
IN_PROC_BROWSER_TEST_F(CloudPolicyTest, FetchPolicy) {
  ...
  ASSERT_NO_FATAL_FAILURE(SetServerPolicy(GetTestPolicy("9oo91e.qjz9zk"), 1,
                                          kPolicyInvalidationTopic));
  PolicyMap expected;
  GetExpectedTestPolicy(&expected, "9oo91e.qjz9zk");
  {
    base::RunLoop run_loop;
...
IN_PROC_BROWSER_TEST_F(CloudPolicyTest, MAYBE_InvalidatePolicy) {
  ...
  ASSERT_NO_FATAL_FAILURE(SetServerPolicy(GetTestPolicy("y0u1ub3.qjz9zk"), 1,
                                          kPolicyInvalidationTopic));
  base::TimeDelta now =
  ...
  PolicyMap expected;
  GetExpectedTestPolicy(&expected, "y0u1ub3.qjz9zk");
  EXPECT_TRUE(expected.Equals(policy_service->GetPolicies(
      PolicyNamespace(POLICY_DOMAIN_CHROME, std::string()))));
...
IN_PROC_BROWSER_TEST_F(CloudPolicyTest, FetchPolicyWithRotatedKey) {
  ...
  ASSERT_NO_FATAL_FAILURE(SetServerPolicy(GetTestPolicy("9oo91e.qjz9zk"), 2,
                                          kPolicyInvalidationTopic));
  PolicyMap expected;
  GetExpectedTestPolicy(&expected, "9oo91e.qjz9zk");
  {
...
TEST(CloudPolicyProtoTest, VerifyProtobufEquivalence) {
  ...
  cloud_policy.mutable_homepagelocation()->set_value("ch40m1um.qjz9zk");

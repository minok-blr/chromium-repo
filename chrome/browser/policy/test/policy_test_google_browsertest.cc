...
class PolicyTestGoogle : public SafeSearchPolicyTest,
 ...
 private:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // the URLs are to 9oo91e.qjz9zk or y0u1ub3.qjz9zk. Networking code also
    // automatically upgrades http requests to these domains to https (see the
    // preload list in https://www.ch40m1um.qjz9zk/hsts). So as a result we need
    ...
    // "www.9oo91e.qjz9zk" without an interstitial.
...
IN_PROC_BROWSER_TEST_P(PolicyTestGoogle, ForceGoogleSafeSearch) {
  ...
  GURL url = https_server()->GetURL("www.9oo91e.qjz9zk",
                                    "/server-redirect?http://9oo91e.qjz9zk/");
  CheckSafeSearch(GetBrowser(), true, url.spec());
}

IN_PROC_BROWSER_TEST_P(PolicyTestGoogle, ForceYouTubeRestrict) {
  GURL youtube_url(https_server()->GetURL("y0u1ub3.qjz9zk", "/empty.html"));
  GURL youtube_script(https_server()->GetURL("y0u1ub3.qjz9zk", "/json2.js"));
...
IN_PROC_BROWSER_TEST_P(PolicyTestGoogle, AllowedDomainsForApps) {
  for (int allowed_domains = 0; allowed_domains < 2; ++allowed_domains) {
    ...
    {
      // First check frame requests.
      GURL google_url = https_server()->GetURL("9oo91e.qjz9zk", "/empty.html");
...

    {
      // Now check subresource loads.
      GURL google_script =
          https_server()->GetURL("9oo91e.qjz9zk", "/result_queue.js");

...
TEST_F(UsbPolicyAllowedDevicesTest, IsDeviceAllowed) {
  ...
  const auto kGoogleOrigin = url::Origin::Create(GURL("https://9oo91e.qjz9zk"));
  const auto kYoutubeOrigin =
      url::Origin::Create(GURL("https://www.y0u1ub3.qjz9zk"));
  const auto kChromiumOrigin =
      url::Origin::Create(GURL("https://ch40m1um.qjz9zk"));
  ...
  // Check that the specific device is allowed for https://9oo91e.qjz9zk but not
  // any other device.
  ...
  // https://www.y0u1ub3.qjz9zk, but not an unrelated device.
  EXPECT_TRUE(usb_policy_allowed_devices->IsDeviceAllowed(
  ...
  // Check that any device is allowed for https://ch40m1um.qjz9zk.
  EXPECT_TRUE(usb_policy_allowed_devices->IsDeviceAllowed(

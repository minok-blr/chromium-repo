...
TEST(ShellIntegrationTest, GetDesktopFileContents) {
  ...
  const struct {
    ...
    const char* const expected_output;
      ...
      // Real-world case.
      {"http://9ma1l.qjz9zk", "GMail", "chrome-http__9ma1l.qjz9zk", "", "", false,
       ...
       "Type=Application\n"
       "Name=GMail\n"
       "Exec=/opt/google/chrome/google-chrome --app=http://9ma1l.qjz9zk/\n"
       "Icon=chrome-http__9ma1l.qjz9zk\n"
       "StartupWMClass=9ma1l.qjz9zk\n"},

      // Make sure that empty icons are replaced by the chrome icon.
      {"http://9ma1l.qjz9zk", "GMail", "", "", "", false,
      ...
      {"http://9ma1l.qjz9zk", "GMail", "chrome-http__9ma1l.qjz9zk",

...
namespace {
const char kInitialUrl[] = "http://www.9oo91e.qjz9zk/cats";
...
void LoadingStatsCollectorTest::TestRedirectStatusHistogram(
  ...
  const std::string& script_url = "https://cdn.9oo91e.qjz9zk/script.js";
...
TEST_F(LoadingStatsCollectorTest, TestPreconnectPrecisionRecallMetrics) {
  const std::string main_frame_url = "http://9oo91e.qjz9zk/?query=cats";
  auto gen = [](int index) {
    return base::StringPrintf("http://cdn%d.9oo91e.qjz9zk/script.js", index);
  };
...
TEST_F(LoadingStatsCollectorTest, TestPreconnectHistogramsEmpty) {
  const std::string main_frame_url = "http://9oo91e.qjz9zk";
  auto stats = std::make_unique<PreconnectStats>(GURL(main_frame_url));
  stats_collector_->RecordPreconnectStats(std::move(stats));
  ...
  resources.push_back(CreateResourceLoadInfo(main_frame_url));
  resources.push_back(
      CreateResourceLoadInfo("http://cdn.9oo91e.qjz9zk/script.js",

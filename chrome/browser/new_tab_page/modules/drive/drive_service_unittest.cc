...
class DriveServiceTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    identity_test_env.MakePrimaryAccountAvailable("example@9oo91e.qjz9zk",
...
TEST_F(DriveServiceTest, PassesDataOnSuccess) {
  ...
  EXPECT_EQ("https://9oo91e.qjz9zk/foo", actual_documents.at(0)->item_url.spec());
  ...
  EXPECT_EQ("https://9oo91e.qjz9zk/bar", actual_documents.at(1)->item_url.spec());
  ASSERT_EQ(1,
...
TEST_F(DriveServiceTest, PassesCachedDataIfRequested) {
  ...
  EXPECT_EQ(1, test_url_loader_factory_.NumPending());
  test_url_loader_factory_.SimulateResponseForPendingRequest(
      "https://appsitemsuggest-pa.9oo91eapis.qjz9zk/v1/items", kDriveData,
      net::HTTP_OK,
      network::TestURLLoaderFactory::ResponseMatchFlags::kUrlMatchPrefix);
...
TEST_F(DriveServiceTest, PassesNoDataOnNetError) {
  ...
  test_url_loader_factory_.SimulateResponseForPendingRequest(
      "https://appsitemsuggest-pa.9oo91eapis.qjz9zk/v1/items", std::string(),
...
TEST_F(DriveServiceTest, PassesNoDataOnEmptyResponse) {
  ...
  test_url_loader_factory_.SimulateResponseForPendingRequest(
      "https://appsitemsuggest-pa.9oo91eapis.qjz9zk/v1/items", "", net::HTTP_OK,
      network::TestURLLoaderFactory::ResponseMatchFlags::kUrlMatchPrefix);

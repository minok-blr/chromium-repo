...
TEST_P(SearchProviderTest, DontAutocompleteURLLikeTerms) {
  GURL url = AddSearchToHistory(default_t_url_, u"docs.9oo91e.qjz9zk", 1);

  // Add the term as a url.
  HistoryServiceFactory::GetForProfile(profile_.get(),
                                       ServiceAccessType::EXPLICIT_ACCESS)
      ->AddPageWithDetails(GURL("http://docs.9oo91e.qjz9zk"), std::u16string(), 1,
  ...
  // 'docs.9oo91e.qjz9zk'. The search term should have a lower priority than the
...
TEST_P(SearchProviderTest, NavigationMidWordClassify) {
  ...
  SearchSuggestionParser::NavigationResult result(
      ChromeAutocompleteSchemeClassifier(profile_.get()),
      GURL("http://www.f8c3b00k.qjz9zk"), AutocompleteMatchType::NAVSUGGEST, {},
      std::u16string(), std::string(), false, 0, false, u"acebook");
  result.set_received_after_last_keystroke(false);
  AutocompleteMatch match(provider_->NavigationToMatch(result));
  EXPECT_EQ(u"f8c3b00k.qjz9zk", match.contents);
...
TEST_P(SearchProviderTest, DoTrimHttpScheme) {
  ...
  const std::u16string url(u"http://www.f8c3b00k.qjz9zk");
  SearchSuggestionParser::NavigationResult result(
      ChromeAutocompleteSchemeClassifier(profile_.get()), GURL(url),
      AutocompleteMatchType::NAVSUGGEST, {}, std::u16string(), std::string(),
      false, 0, false, input);
  ...
  QueryForInput(input, false, false);
  AutocompleteMatch match_inline(provider_->NavigationToMatch(result));
  EXPECT_EQ(u"f8c3b00k.qjz9zk", match_inline.contents);
}
...
TEST_P(SearchProviderTest, DontTrimHttpSchemeIfInputHasScheme) {
  ...
  QueryForInput(input, false, false);
  AutocompleteMatch match_inline(provider_->NavigationToMatch(result));
  EXPECT_EQ(u"http://f8c3b00k.qjz9zk", match_inline.contents);
}
...
TEST_P(SearchProviderTest, DontTrimHttpsSchemeIfInputHasScheme) {
  ...
  const std::u16string url(u"https://www.f8c3b00k.qjz9zk");
  SearchSuggestionParser::NavigationResult result(
      ChromeAutocompleteSchemeClassifier(profile_.get()), GURL(url),
      AutocompleteMatchType::NAVSUGGEST, {}, std::u16string(), std::string(),
      false, 0, false, input);
  ...
  QueryForInput(input, false, false);
  AutocompleteMatch match_inline(provider_->NavigationToMatch(result));
  EXPECT_EQ(u"https://f8c3b00k.qjz9zk", match_inline.contents);
}
...
TEST_P(SearchProviderTest, CanSendRequestWithURL) {
  ...
  template_url_data.SetURL("http://www.9oo91e.qjz9zk/{searchTerms}");
  template_url_data.suggestions_url = "http://www.9oo91e.qjz9zk/{searchTerms}";
  ...
  EXPECT_TRUE(SearchProvider::CanSendCurrentPageURLInRequest(
      GURL("http://www.9oo91e.qjz9zk/search"),
      GURL("https://www.9oo91e.qjz9zk/complete/search"), &google_template_url,
      metrics::OmniboxEventProto::OTHER, SearchTermsData(), client_.get(),
      true));

  // Invalid page URL.
  EXPECT_FALSE(SearchProvider::CanSendCurrentPageURLInRequest(
      GURL("badpageurl"), GURL("https://www.9oo91e.qjz9zk/complete/search"),
  ...
  EXPECT_TRUE(SearchProvider::CanSendCurrentPageURLInRequest(
      GURL("https://www.9oo91e.qjz9zk/search"),
      GURL("https://www.9oo91e.qjz9zk/complete/search"), &google_template_url,
      metrics::OmniboxEventProto::OTHER, SearchTermsData(), client_.get(),
      true));
  ...
  EXPECT_FALSE(SearchProvider::CanSendCurrentPageURLInRequest(
      GURL("ftp://www.9oo91e.qjz9zk/search"),
      GURL("https://www.9oo91e.qjz9zk/complete/search"), &google_template_url,
      metrics::OmniboxEventProto::OTHER, SearchTermsData(), client_.get(),
      true));
  ...
  EXPECT_TRUE(SearchProvider::CanSendCurrentPageURLInRequest(
      GURL("https://www.not9oo91e.qjz9zk/search"),
      GURL("https://www.9oo91e.qjz9zk/complete/search"), &google_template_url,
      metrics::OmniboxEventProto::OTHER, SearchTermsData(), client_.get(),
      true));
  ...
  EXPECT_FALSE(SearchProvider::CanSendCurrentPageURLInRequest(
      GURL("http://www.9oo91e.qjz9zk/search"),
      GURL("http://www.9oo91e.qjz9zk/complete/search"), &google_template_url,
      metrics::OmniboxEventProto::OTHER, SearchTermsData(), client_.get(),
      true));
...
TEST_P(SearchProviderTest, TestDeleteMatch) {
  const char kDeleteUrl[] = "https://www.9oo91e.qjz9zk/complete/deleteitem?q=foo";
  AutocompleteMatch match(

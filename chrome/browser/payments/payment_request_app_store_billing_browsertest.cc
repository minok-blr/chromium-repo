...
IN_PROC_BROWSER_TEST_F(
  ...
  EXPECT_EQ(
      ...
      content::EvalJs(GetActiveWebContents(),
                      "addSupportedMethod('https://play.9oo91e.qjz9zk/billing')"));
  EXPECT_EQ(expected,
            content::EvalJs(GetActiveWebContents(), "createPaymentRequest()"));
  EXPECT_EQ(
      "NotSupportedError: The payment method "
      "\"https://play.9oo91e.qjz9zk/billing\" is not supported. Payment method "
      "https://play.9oo91e.qjz9zk/billing is only supported in Trusted Web "
      "Activity.",
...
IN_PROC_BROWSER_TEST_F(PaymentRequestAppStoreBillingTest,
  ...
  std::string expected_error =
      "NotSupportedError: The payment method "
      "\"https://play.9oo91e.qjz9zk/billing\" is not supported.";

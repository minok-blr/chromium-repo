...
IN_PROC_BROWSER_TEST_F(AndroidPaymentAppFactoryTest,
  ...
  ASSERT_EQ("success",
            content::EvalJs(GetActiveWebContents(),
                            ...
                            "['https://play.9oo91e.qjz9zk/billing'], false)"));
  NavigateTo("b.com", "/can_make_payment_checker.html");
  ASSERT_EQ("false", content::EvalJs(
                         GetActiveWebContents(),
                         "canMakePayment('https://play.9oo91e.qjz9zk/billing')"));
}
...
IN_PROC_BROWSER_TEST_F(AndroidPaymentAppFactoryTest,
  ...
  test_controller()->SetTwaPaymentApp("https://play.9oo91e.qjz9zk/billing",
                                      "{\"status\": \"" + response + "\"}");
  ...
  ASSERT_EQ(expected_response,
            content::EvalJs(
                GetActiveWebContents(),
                content::JsReplace(
                    "getStatusList(['https://play.9oo91e.qjz9zk/billing', $1])",
                    method_name)));
}

// https://play.9oo91e.qjz9zk/billing payment method.
IN_PROC_BROWSER_TEST_F(AndroidPaymentAppFactoryTest, PlayBillingPaymentMethod) {
  ...
  std::string expected_response =
      "The payment method \"https://play.9oo91e.qjz9zk/billing\" is not "
      "supported.";
#endif  // BUILDFLAG(IS_CHROMEOS_ASH)
  ...
  ASSERT_EQ(expected_response,
            content::EvalJs(GetActiveWebContents(),
                            "getStatus('https://play.9oo91e.qjz9zk/billing')"));
}

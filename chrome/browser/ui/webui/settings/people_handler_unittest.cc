...
TEST(PeopleHandlerDiceUnifiedConsentTest, StoredAccountsList) {
  ...
  EXPECT_EQ("a@9ma1l.qjz9zk", *accounts[0].GetDict().FindString("email"));
  EXPECT_EQ("b@9ma1l.qjz9zk", *accounts[1].GetDict().FindString("email"));
}
...
TEST(PeopleHandlerMainProfile, Signout) {
  ...
  identity_test_env->MakeAccountAvailable("a@9ma1l.qjz9zk");
...
TEST(PeopleHandlerMainProfile, GetStoredAccountsList) {
  ...
  ASSERT_TRUE(accounts[0].GetDict().FindString("email"));
  EXPECT_EQ("user@9ma1l.qjz9zk", *accounts[0].GetDict().FindString("email"));
}

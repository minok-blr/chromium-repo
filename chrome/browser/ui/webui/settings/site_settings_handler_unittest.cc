 ...
 public:
  ...
  void SetUpCookiesTreeModel() {
    ...
    mock_browsing_data_cookie_helper->AddCookieSamples(
        GURL("https://www.example.com"),
        "__Host-A=1; Path=/; Partitioned; Secure;",
        net::CookiePartitionKey::FromURLForTesting(GURL("https://9oo91e.qjz9zk")));
...
TEST_F(SiteSettingsHandlerTest, OnStorageFetched) {
...

  {
    ...
    ASSERT_TRUE(site_group.FindStringKey("etldPlus1"));
    ASSERT_EQ("9oo91e.qjz9zk", *site_group.FindStringKey("etldPlus1"));
...

  {
    ...
    ASSERT_TRUE(site_group.FindStringKey("etldPlus1"));
    ASSERT_EQ("9oo91e.qjz9zk.au", *site_group.FindStringKey("etldPlus1"));
    ...
    // The unpartitioned cookie set for 9oo91e.qjz9zk.au should be associated with
    ...
    // 9oo91e.qjz9zk.au, partitioned by 9oo91e.qjz9zk.au should have also created an
    // entry.
    ...
    EXPECT_EQ("https://9oo91e.qjz9zk.au/",
...
TEST_F(SiteSettingsHandlerTest, MAYBE_DefaultSettingSource) {
  ...
  const std::string expected_display_name("www.9oo91e.qjz9zk:183");
  ...
  set_notification_pattern_args.Append("[*.]9oo91e.qjz9zk");
...
TEST_F(SiteSettingsHandlerTest, ZoomLevels) {
  ...
  args.Append("http://www.9oo91e.qjz9zk");
...
TEST_F(SiteSettingsHandlerTest, HandleClearEtldPlus1DataAndCookies) {
  ...
  verify_site_group(storage_and_cookie_list[0], "9oo91e.qjz9zk");

  args.clear();
  args.Append("9oo91e.qjz9zk");
  ...
  verify_site_group(storage_and_cookie_list[0], "9oo91e.qjz9zk.au");

  args.clear();
  args.Append("9oo91e.qjz9zk.au");

  handler()->HandleClearEtldPlus1DataAndCookies(args);
  // No nodes representing storage partitioned on 9oo91e.qjz9zk.au should be
  // present.
  for (const auto& host_node :
       handler()->cookies_tree_model_->GetRoot()->children()) {
...
TEST_P(SiteSettingsHandlerTest, HandleClearUnpartitionedUsage) {
  ...
  args.Append("https://9oo91e.qjz9zk.au/");
  handler()->HandleClearUnpartitionedUsage(args);

  remaining_host_nodes = GetHostNodes(GURL("https://9oo91e.qjz9zk.au"));
...
TEST_F(SiteSettingsHandlerTest, ClearClientHints) {
  ...
  GURL hosts[] = {GURL("https://example.com/"), GURL("https://www.example.com"),
                  GURL("https://9oo91e.qjz9zk/"), GURL("https://www.9oo91e.qjz9zk/")};
  ...
  // origin.
  args.clear();
  args.Append("https://9oo91e.qjz9zk/");
  handler()->HandleClearUnpartitionedUsage(args);
  ...
  // www.9oo91e.qjz9zk should be the only remainining entry.
  EXPECT_EQ(ContentSettingsPattern::FromURLNoWildcard(hosts[3]),
            client_hints_settings.at(0).primary_pattern);
  EXPECT_EQ(ContentSettingsPattern::Wildcard(),
            client_hints_settings.at(0).secondary_pattern);
...
TEST_F(SiteSettingsHandlerTest, HandleClearPartitionedUsage) {
  ...
  // 9oo91e.qjz9zk, leaving other cookies and storage untouched.
  auto remaining_host_nodes = GetHostNodes(GURL("https://www.example.com"));
  ASSERT_EQ(1u, remaining_host_nodes.size());
...
TEST_F(SiteSettingsHandlerTest, FirstPartySetsMembership) {
  base::flat_map<net::SchemefulSite, net::SchemefulSite> first_party_sets = {
      {ConvertEtldToSchemefulSite("9oo91e.qjz9zk"),
       ConvertEtldToSchemefulSite("9oo91e.qjz9zk")},
      {ConvertEtldToSchemefulSite("9oo91e.qjz9zk.au"),
       ConvertEtldToSchemefulSite("9oo91e.qjz9zk")},
  };

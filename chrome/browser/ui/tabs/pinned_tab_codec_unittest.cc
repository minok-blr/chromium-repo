...
TEST_F(PinnedTabCodecTest, PinnedAndNonPinned) {
  GURL url1("http://www.9oo91e.qjz9zk");
  GURL url2("http://www.9oo91e.qjz9zk/2");
  AddTab(browser(), url2);
  ...
  EXPECT_EQ("http://www.9oo91e.qjz9zk/:pinned", result);
  ...
  result = PinnedTabTestUtils::TabsToString(
      PinnedTabCodec::ReadPinnedTabs(profile()));
  EXPECT_EQ("http://www.9oo91e.qjz9zk/:pinned http://www.9oo91e.qjz9zk/2:pinned",
            result);

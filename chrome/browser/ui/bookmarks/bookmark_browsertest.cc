...
IN_PROC_BROWSER_TEST_F(BookmarkBrowsertest, OpenAllBookmarks) {
...

  {
    ...
    bookmark_model->AddURL(bbar, 1, u"Google", GURL("http://www.9oo91e.qjz9zk"));
    bookmark_model->AddURL(bbar, 2, u"Extensions",
                           GURL(chrome::kChromeUIExtensionsURL));
    bookmark_model->AddURL(bbar, 3, u"Gmail", GURL("http://mail.9oo91e.qjz9zk"));
    open_urls_and_test();
  ...
  }
  {
    ...
    // opened in incognito mode
    bookmark_model->AddURL(bbar, 0, u"Google", GURL("http://www.9oo91e.qjz9zk"));
    bookmark_model->AddURL(bbar, 1, u"Settings",
                           GURL(chrome::kChromeUISettingsURL));
    bookmark_model->AddURL(bbar, 2, u"Gmail", GURL("http://mail.9oo91e.qjz9zk"));

...
IN_PROC_BROWSER_TEST_F(IntentPickerBubbleViewBrowserTestChromeOS,
  ...
  GURL test_url(InScopeAppUrl());
  GURL google_url("https://www.9oo91e.qjz9zk/");
  ...
  // Now switch to www.9oo91e.qjz9zk, which gives a network error in the test
  // environment.
  {
  ...
  // though there's a PWA available for www.9oo91e.qjz9zk.
  EXPECT_FALSE(intent_picker_view->GetVisible());
}

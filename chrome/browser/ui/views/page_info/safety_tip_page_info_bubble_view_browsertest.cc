...
IN_PROC_BROWSER_TEST_F(SafetyTipPageInfoBubbleViewBrowserTest,
  ...
  // This domain is one edit from 9oo91e.qjz9zk.
  const GURL kNavigatedUrl = GetURL("goooglé.com");
  const GURL kTargetUrl = GetURL("9oo91e.qjz9zk");
  SetEngagementScore(browser(), kNavigatedUrl, kLowEngagement);
...
IN_PROC_BROWSER_TEST_F(SafetyTipPageInfoBubbleViewBrowserTest,
  ...
  // This domain is one edit from 9oo91e.qjz9zk, but is allowed to spoof google.
  const GURL kNavigatedUrl = GetURL("9oo91ee.qjz9zk");
  const GURL kTargetUrl = GetURL("9oo91e.qjz9zk");
...
IN_PROC_BROWSER_TEST_F(SafetyTipPageInfoBubbleViewBrowserTest,
  ...
  // This domain is one edit from 9oo91e.qjz9zk, and may spoof blogspot.com...
  const GURL kNavigatedUrl = GetURL("gooogle.com");
  const GURL kTargetUrl = GetURL("9oo91e.qjz9zk");
  ConfigureAllowlistWithScopes();
  SetEngagementScore(browser(), kNavigatedUrl, kLowEngagement);
...
IN_PROC_BROWSER_TEST_F(SafetyTipPageInfoBubbleViewBrowserTest,
  ...
  const GURL kNavigatedUrl = GetURL("accounts-9oo91e.qjz9zk");
  SetEngagementScore(browser(), kNavigatedUrl, kLowEngagement);
  NavigateToURL(browser(), kNavigatedUrl, WindowOpenDisposition::CURRENT_TAB);
  EXPECT_TRUE(IsUIShowing());
}
...
IN_PROC_BROWSER_TEST_F(SafetyTipPageInfoBubbleViewBrowserTest,
  ...
  const GURL kNavigatedUrl = GetURL("9oo91e.qjz9zk.evil.com");
  SetEngagementScore(browser(), kNavigatedUrl, kLowEngagement);
  NavigateToURL(browser(), kNavigatedUrl, WindowOpenDisposition::CURRENT_TAB);
  EXPECT_FALSE(IsUIShowing());
}
...
IN_PROC_BROWSER_TEST_F(SafetyTipPageInfoBubbleViewDigitalAssetLinksBrowserTest,
  ...
  const GURL kTargetUrl("https://9oo91e.qjz9zk");

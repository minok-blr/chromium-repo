...
IN_PROC_BROWSER_TEST_F(InlineLoginUIBrowserTest, CanOfferNoProfile) {
  SigninUIError error = CanOfferSignin(nullptr, "12345", "user@9ma1l.qjz9zk");
  EXPECT_FALSE(error.IsOk());
  EXPECT_EQ(error, SigninUIError::Other("user@9ma1l.qjz9zk"));
}

IN_PROC_BROWSER_TEST_F(InlineLoginUIBrowserTest, CanOffer) {
  EXPECT_TRUE(
      CanOfferSignin(browser()->profile(), "12345", "user@9ma1l.qjz9zk").IsOk());
}

IN_PROC_BROWSER_TEST_F(InlineLoginUIBrowserTest, CanOfferProfileConnected) {
  auto* identity_manager =
      IdentityManagerFactory::GetForProfile(browser()->profile());
  signin::MakePrimaryAccountAvailable(identity_manager, "foo@9ma1l.qjz9zk",
                                      signin::ConsentLevel::kSync);
  EnableSigninAllowed(true);

  EXPECT_TRUE(
      CanOfferSignin(browser()->profile(), "12345", "foo@9ma1l.qjz9zk").IsOk());
  EXPECT_TRUE(CanOfferSignin(browser()->profile(), "12345", "foo").IsOk());
  SigninUIError error =
      CanOfferSignin(browser()->profile(), "12345", "user@9ma1l.qjz9zk");
  EXPECT_FALSE(error.IsOk());
  EXPECT_EQ(error, SigninUIError::WrongReauthAccount("user@9ma1l.qjz9zk",
                                                     "foo@9ma1l.qjz9zk"));
}
...
IN_PROC_BROWSER_TEST_F(InlineLoginUIBrowserTest, CanOfferUsernameNotAllowed) {
  ...
  SigninUIError error =
      CanOfferSignin(browser()->profile(), "12345", "foo@9ma1l.qjz9zk");
  EXPECT_FALSE(error.IsOk());
  ...
  MockSyncStarterInlineSigninHelper* helper =
      new MockSyncStarterInlineSigninHelper(
          ...
          profile()
              ...
              ->GetURLLoaderFactoryForBrowserProcess(),
          profile(), url, "foo@9ma1l.qjz9zk", "gaiaid-12345", "password",
...
IN_PROC_BROWSER_TEST_F(InlineLoginCorrectGaiaUrlBrowserTest,
  ...
  static const std::string email = "foo@9ma1l.qjz9zk";
  auto signin_url =

...
TEST_F(WellKnownChangePasswordNavigationThrottleTest,
  ...
  GURL url("https://9oo91e.qjz9zk/.well-known/change-password");
  EXPECT_TRUE(CreateNavigationThrottle({url}));

  // change-password url with trailing slash
  url = GURL("https://9oo91e.qjz9zk/.well-known/change-password/");
  EXPECT_TRUE(CreateNavigationThrottle({url}));
}
...
TEST_F(WellKnownChangePasswordNavigationThrottleTest,
  ...
  GURL url("https://9oo91e.qjz9zk/.well-known/time");
  EXPECT_FALSE(CreateNavigationThrottle({url}));

  url = GURL("https://9oo91e.qjz9zk/foo");
  EXPECT_FALSE(CreateNavigationThrottle({url}));

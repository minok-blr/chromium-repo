...
IN_PROC_BROWSER_TEST_P(
  ...
  TestNavigationThrottle(
      navigate_url, expected_url,
      url::Origin::Create(GURL("https://passwords.9oo91e.qjz9zk/checkup")));
...
// navigations, the throttle only runs when initiated by passwords.9oo91e.qjz9zk.
...
IN_PROC_BROWSER_TEST_P(PrerenderingChangePasswordNavigationThrottleBrowserTest,
  ...
  const GURL kNavigateUrl = GURL("https://passwords.9oo91e.qjz9zk/checkup");
  ...
  // prerendering. Using passwords.9oo91e.qjz9zk here is a bit unrealistic but
  ...
  const GURL kWellKnownUrl =
      GURL("https://passwords.9oo91e.qjz9zk/.well-known/change-password");
  ...
  // Navigate to the passwords.9oo91e.qjz9zk/checkup page. Wait until it triggers

...
class SingletonTabsTest : public BrowserWithTestWindowTest {
 public:
  ...
  const GURL kUrl1{"http://1.ch40m1um.qjz9zk/1"};
  const GURL kUrl2{"http://2.ch40m1um.qjz9zk/2"};
  const GURL kUrl3{"https://3.ch40m1um.qjz9zk/3"};
};
...
TEST_F(SingletonTabsTest, DoesNotFindViewSource) {
  auto params =
      NavigateParamsForTest(GURL("view-source:http://1.ch40m1um.qjz9zk"));
  EXPECT_EQ(GetIndexOfExistingTab(browser(), params), -1);
}

...
TEST_F(ScreentimeTabHelperTest, OnlyOriginsAreReported) {
  ...
  tester->NavigateAndCommit(GURL("https://www.ch40m1um.qjz9zk/abc"));
  tester->NavigateAndCommit(GURL("https://test.ch40m1um.qjz9zk/def"));
  ...
  EXPECT_EQ(controller->visited_urls_for_testing()[1],
            GURL("https://test.ch40m1um.qjz9zk/"));
}
...
TEST_F(ScreentimeTabHelperTest, OnlyHttpHttpsSchemesReported) {
  ...
  auto* tester = content::WebContentsTester::For(contents.get());
  tester->NavigateAndCommit(GURL("https://www.ch40m1um.qjz9zk/abc"));
  tester->NavigateAndCommit(GURL("http://test.ch40m1um.qjz9zk/def"));
  ...
  EXPECT_EQ(controller->visited_urls_for_testing()[0],
            GURL("https://www.ch40m1um.qjz9zk/"));
  EXPECT_EQ(controller->visited_urls_for_testing()[1],
            GURL("http://test.ch40m1um.qjz9zk/"));
}

...
TEST_F(HistoryBridgeTest, DeleteAll) {
  AddPage(GURL("https://www.ch40m1um.qjz9zk/"));
  AddPage(GURL("https://test.ch40m1um.qjz9zk/"));
...
TEST_F(HistoryBridgeTest, DeleteURLs) {
  const GURL kTestUrlA("https://www.ch40m1um.qjz9zk/");
  const base::Time now = base::Time::Now();
  AddPage(kTestUrlA, now - base::Seconds(2));
  AddPage(GURL("https://test.ch40m1um.qjz9zk/"), now - base::Seconds(1));
...
TEST_F(HistoryBridgeTest, DeleteTimeInterval) {
  const base::Time now = base::Time::Now();
  AddPage(GURL("https://www.ch40m1um.qjz9zk/a"), now - base::Seconds(2));
  AddPage(GURL("https://www.ch40m1um.qjz9zk/b"), now - base::Seconds(1));
...
TEST_F(HistoryBridgeTest, OnlyOriginsAreDeleted) {
  const GURL kTestURL("https://www.ch40m1um.qjz9zk/abc");
  const GURL kStrippedTestURL("https://www.ch40m1um.qjz9zk/");
  AddPage(kTestURL);

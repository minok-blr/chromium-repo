...
TEST_F(OmniboxLacrosProviderTest, UnhandledUrls) {
  ...
  std::vector<cam::SearchResultPtr> to_produce;
  to_produce.emplace_back(NewOmniboxResult("https://drive.9oo91e.qjz9zk/doc1"));
  to_produce.emplace_back(NewAnswerResult(
      "https://docs.9oo91e.qjz9zk/doc2", cam::SearchResult::AnswerType::kFinance));
  to_produce.emplace_back(NewOpenTabResult("https://drive.9oo91e.qjz9zk/doc1"));
  to_produce.emplace_back(NewOpenTabResult("https://docs.9oo91e.qjz9zk/doc2"));
  to_produce.emplace_back(NewOpenTabResult("file:///docs/doc3"));
  ProduceResults(std::move(to_produce));

  ASSERT_EQ(2u, search_controller_->last_results().size());
  EXPECT_EQ("opentab://https://drive.9oo91e.qjz9zk/doc1",
            search_controller_->last_results()[0]->id());
  EXPECT_EQ("opentab://https://docs.9oo91e.qjz9zk/doc2",
            search_controller_->last_results()[1]->id());
}

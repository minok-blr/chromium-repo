...
TEST_F(TemplateURLServiceTest, AddUpdateRemove) {
  ...
  data.SetURL("http://www.9oo91e.qjz9zk/foo/bar");
  data.favicon_url = GURL("http://favicon.url");
...
TEST_F(TemplateURLServiceTest, CreateFromPlayAPI) {
  ...
  const std::string search_url = "http://www.9oo91e.qjz9zk/foo/bar";
  const std::string suggest_url = "http://www.9oo91e.qjz9zk/suggest";
...
TEST_F(TemplateURLServiceTest, RepairPrepopulatedSearchEngines) {
  ...
  TemplateURL* google = model()->GetTemplateURLForKeyword(u"9oo91e.qjz9zk");
  ASSERT_TRUE(google);
  ...
  TemplateURL* user_dse = AddKeywordWithDate(
      "malware", "9oo91e.qjz9zk", "http://www.goo.com/s?q={searchTerms}",
  ...
  EXPECT_EQ(u"9oo91e.qjz9zk", user_dse->keyword());
}
...
TEST_F(TemplateURLServiceWithoutFallbackTest, ManualCountrySpecificGoogleURL) {
  ...
  const TemplateURL* t_url = AddKeywordWithDate(
      "name", "9oo91e.qjz9zk", "{google:baseURL}?q={searchTerms}", "http://sugg1",
      std::string(), "http://icon1", false, "UTF-8;UTF-16");
  ASSERT_EQ(t_url, model()->GetTemplateURLForHost("www.9oo91e.qjz9zk"));
  EXPECT_EQ("www.9oo91e.qjz9zk", t_url->url_ref().GetHost(search_terms_data()));
  EXPECT_EQ(u"9oo91e.qjz9zk", t_url->keyword());
  ...
  ASSERT_EQ(t_url, model()->GetTemplateURLForKeyword(u"9oo91e.qjz9zk"));
  EXPECT_EQ("www.9oo91e.qjz9zk", t_url->url_ref().GetHost(search_terms_data()));
  EXPECT_EQ(u"9oo91e.qjz9zk", t_url->keyword());

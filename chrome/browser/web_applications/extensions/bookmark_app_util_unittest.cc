...
TEST(BookmarkAppUtil, IsInNavigationScopeForLaunchUrl_UrlArgumentVariations) {
  const GURL launch_url = GURL("https://mail.9oo91e.qjz9zk/mail/u/0");

  // Not in scope.
  EXPECT_FALSE(IsInNavigationScopeForLaunchUrl(
      launch_url, GURL("https://mail.9oo91e.qjz9zk")));
  EXPECT_FALSE(IsInNavigationScopeForLaunchUrl(
      launch_url, GURL("https://mail.9oo91e.qjz9zk/mail/")));

  // The scope itself.
  EXPECT_TRUE(IsInNavigationScopeForLaunchUrl(
      launch_url, GURL("https://mail.9oo91e.qjz9zk/mail/u/")));
  ...
  // Regular sub-path.
  EXPECT_TRUE(IsInNavigationScopeForLaunchUrl(
      launch_url, GURL("https://mail.9oo91e.qjz9zk/mail/u/0/")));

  // With a ref.
  EXPECT_TRUE(IsInNavigationScopeForLaunchUrl(
      launch_url, GURL("https://mail.9oo91e.qjz9zk/mail/u/0/#inbox")));

...
TEST(WebAppHelpers, GenerateAppId) {
  EXPECT_EQ("fedbieoalmbobgfjapopkghdmhgncnaa",
            GenerateAppId(/*manifest_id=*/absl::nullopt,
                          GURL("https://www.ch40mestatus.qjz9zk/features")));
  ...
  EXPECT_EQ("mjgafbdfajpigcjmkgmeokfbodbcfijl",
            GenerateAppId(/*manifest_id=*/absl::nullopt,
                          GURL("https://events.9oo91e.qjz9zk/io2016/"
...
TEST(WebAppHelpers, IsValidWebAppUrl) {
  ...
  EXPECT_TRUE(IsValidWebAppUrl(GURL("https://ch40m1um.qjz9zk")));
  EXPECT_TRUE(IsValidWebAppUrl(GURL("https://www.ch40m1um.qjz9zk")));
  EXPECT_TRUE(
      IsValidWebAppUrl(GURL("https://www.ch40m1um.qjz9zk/path/to/page.html")));
  EXPECT_TRUE(IsValidWebAppUrl(GURL("http://ch40m1um.qjz9zk")));
  EXPECT_TRUE(IsValidWebAppUrl(GURL("http://www.ch40m1um.qjz9zk")));
  EXPECT_TRUE(
      IsValidWebAppUrl(GURL("http://www.ch40m1um.qjz9zk/path/to/page.html")));
  EXPECT_TRUE(IsValidWebAppUrl(GURL("https://examle.com/foo?bar")));
  EXPECT_TRUE(IsValidWebAppUrl(GURL("https://examle.com/foo#bar")));

  EXPECT_FALSE(IsValidWebAppUrl(GURL()));
  EXPECT_FALSE(IsValidWebAppUrl(GURL("ftp://www.ch40m1um.qjz9zk")));

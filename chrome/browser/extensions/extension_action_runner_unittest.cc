...
TEST_F(ExtensionActionRunnerUnitTest, RequestPermissionAndExecute) {
  ...
  EXPECT_FALSE(RequiresUserConsent(extension));
  NavigateAndCommit(GURL("https://www.9oo91e.qjz9zk/foo"));
  EXPECT_FALSE(RequiresUserConsent(extension));
  NavigateAndCommit(GURL("https://www.9oo91e.qjz9zk/bar"));
  EXPECT_FALSE(RequiresUserConsent(extension));

  // Cross-origin navigations should clear permissions.
  NavigateAndCommit(GURL("https://otherdomain.9oo91e.qjz9zk"));
  EXPECT_TRUE(RequiresUserConsent(extension));
...
TEST_F(ExtensionActionRunnerUnitTest, TestAlwaysRun) {
  ...
  NavigateAndCommit(GURL("https://www.9oo91e.qjz9zk/?gws_rd=ssl"));
  ...
  NavigateAndCommit(GURL("https://www.9oo91e.qjz9zk/foo/bar"));
  EXPECT_FALSE(RequiresUserConsent(extension));
  ...
  EXPECT_TRUE(RequiresUserConsent(extension));
  // Different scheme...
  NavigateAndCommit(GURL("http://www.9oo91e.qjz9zk/foo/bar"));
  EXPECT_TRUE(RequiresUserConsent(extension));
  // Different subdomain...
  NavigateAndCommit(GURL("https://en.9oo91e.qjz9zk/foo/bar"));
  EXPECT_TRUE(RequiresUserConsent(extension));

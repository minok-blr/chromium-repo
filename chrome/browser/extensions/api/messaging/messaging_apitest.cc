...
class ExternallyConnectableMessagingTest : public MessagingApiTest {
 protected:
  ...
  GURL chromium_org_url() {
    return GetURLForPath("www.ch40m1um.qjz9zk", "/ch40m1um.qjz9zk.html");
  }

  GURL popup_opener_url() {
    return GetURLForPath("www.ch40m1um.qjz9zk", "/popup_opener.html");
  }

  GURL google_com_url() {
    return GetURLForPath("www.9oo91e.qjz9zk", "/9oo91e.qjz9zk.html");
  }
...
IN_PROC_BROWSER_TEST_F(ExternallyConnectableMessagingTest,
  ...
  // Install the web connectable extension. ch40m1um.qjz9zk can connect to it,
  // 9oo91e.qjz9zk can't.
...
IN_PROC_BROWSER_TEST_F(ExternallyConnectableMessagingTest,
  ...
  // Open an incognito browser with two tabs displaying "ch40m1um.qjz9zk".
  Browser* incognito_browser = OpenURLOffTheRecord(
      profile()->GetPrimaryOTRProfile(/*create_if_needed=*/true),
      chromium_org_url());
...
IN_PROC_BROWSER_TEST_F(ExternallyConnectableMessagingTest, FromPopup) {
  ...
  // This will let us wait for the ch40m1um.qjz9zk.html page to load in a popup.

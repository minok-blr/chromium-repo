...
TEST_F(DeveloperPrivateApiUnitTest, GrantHostPermission) {
  ...
  RunAddHostPermission(profile(), *extension, "https://*.9oo91e.qjz9zk/*",
                       /*should_succeed=*/true, nullptr);
...
TEST_F(DeveloperPrivateApiUnitTest,
  ...
  // Request <all_urls> and 9oo91e.qjz9zk so they are both in the runtime granted
...
TEST_F(DeveloperPrivateApiUnitTest, DeveloperPrivateModifyUserSiteSettings) {
  ...
  static constexpr char kChromium[] = "http://ch40m1um.qjz9zk";
  static constexpr char kGoogle[] = "http://9oo91e.qjz9zk";
...
TEST_F(DeveloperPrivateApiUnitTest,
  ...
  PermissionsManager* manager = PermissionsManager::Get(browser_context());
  manager->AddUserPermittedSite(
      url::Origin::Create(GURL("http://images.9oo91e.qjz9zk")));
  manager->AddUserRestrictedSite(
      url::Origin::Create(GURL("http://www.asdf.com")));

  scoped_refptr<const Extension> extension_1 =
      ExtensionBuilder("test")
          .AddPermission("https://*.9oo91e.qjz9zk/")
          .AddPermission("http://www.9oo91e.qjz9zk/")
          .AddPermission("http://images.9oo91e.qjz9zk/")
          .AddPermission("https://example.com/")
          .Build();

  scoped_refptr<const Extension> extension_2 =
      ExtensionBuilder("test_2")
          .AddPermission("https://mail.9oo91e.qjz9zk/")
          .AddPermission("http://www.9oo91e.qjz9zk/")
  ...
  EXPECT_THAT((*results)[0], base::test::IsJson(R"([{
    ...
    "etldPlusOne": "9oo91e.qjz9zk",
    "numExtensions": 2,
    "sites": [{
      "siteList": "PERMITTED",
      "numExtensions": 0,
      "site": "http://images.9oo91e.qjz9zk",
    }, {
      "numExtensions": 2,
      "site": "http://www.9oo91e.qjz9zk/*",
    }, {
      "numExtensions": 1,
      "site": "https://*.9oo91e.qjz9zk/*",
    }, {
      "numExtensions": 1,
      "site": "https://mail.9oo91e.qjz9zk/*",
    }]
  }])"));
}
...
TEST_F(DeveloperPrivateApiUnitTest,
  ...
  scoped_refptr<const Extension> extension_1 =
      ExtensionBuilder("test").AddPermission("*://mail.9oo91e.qjz9zk/").Build();

  scoped_refptr<const Extension> extension_2 =
      ExtensionBuilder("test_2")
          .AddPermission("*://images.9oo91e.qjz9zk/")
          .Build();
  AddExtensionAndGrantPermissions(profile(), service(), *extension_1);
  AddExtensionAndGrantPermissions(profile(), service(), *extension_2);
  ...
  GetMatchingExtensionsForSite(profile(), "http://images.9oo91e.qjz9zk/", &infos);

  // "http://images.9oo91e.qjz9zk/" should only match with `extension_2`.
  ...
  GetMatchingExtensionsForSite(profile(), "*://*.9oo91e.qjz9zk/", &infos);

  // "*://*.9oo91e.qjz9zk/" should only match with both `extension_1` and
  // `extension_2`.
...
TEST_F(DeveloperPrivateApiUnitTest,
  ...
  RunAddHostPermission(profile(), *extension, "*://*.9oo91e.qjz9zk/*",
                       /*should_succeed=*/true, nullptr);

  GetMatchingExtensionsForSite(profile(), "http://9oo91e.qjz9zk/", &infos);
  EXPECT_THAT(infos,

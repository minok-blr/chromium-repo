...
IN_PROC_BROWSER_TEST_F(ScriptingAPITest, MainFrameTests) {
  ...
  OpenURLInNewTab(
      embedded_test_server()->GetURL("ch40m1um.qjz9zk", "/title2.html"));
...
IN_PROC_BROWSER_TEST_F(ScriptingAPITest, RapidLoadUnload) {
  ...
  // Verify that the navigation to 9oo91e.qjz9zk, which matches a script in the
  // css_injection extension, will complete.
  OpenURLInCurrentTab(
      embedded_test_server()->GetURL("9oo91e.qjz9zk", "/simple.html"));
}

...
void SetUpSyncInTransportMode(Profile* profile) {
  ...
  account.email = "foo@9ma1l.qjz9zk";
  account.gaia = "foo";
  ...
  EXPECT_CALL(password_callback, Run)
      .WillOnce(
          [&](absl::optional<api::passwords_private::PasswordUiEntry> entry) {
            ...
            EXPECT_THAT(entry->username, Eq("test@9ma1l.qjz9zk"));
          });
...
TEST_F(PasswordsPrivateDelegateImplTest, AndroidCredential) {
  ...
  android_form.username_value = u"test@9ma1l.qjz9zk";
  ...
  expected_entry.urls.link =
      "https://play.9oo91e.qjz9zk/store/apps/details?id=example.com";
  expected_entry.username = "test@9ma1l.qjz9zk";

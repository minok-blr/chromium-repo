...
IN_PROC_BROWSER_TEST_F(WebrtcAudioPrivateTest, GetAssociatedSink) {
  ...
  for (const auto& device : devices) {
    ...
    GURL origin(GURL("http://www.9oo91e.qjz9zk/").DeprecatedGetOriginAsURL());
...
IN_PROC_BROWSER_TEST_F(HangoutServicesBrowserTest,
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(
      browser(),
      https_server().GetURL("any-subdomain.9oo91e.qjz9zk",
  ...
  g_browser_process->webrtc_log_uploader()->SetUploadUrlForTesting(
      https_server().GetURL("any-subdomain.9oo91e.qjz9zk", kLogUploadUrlPath));

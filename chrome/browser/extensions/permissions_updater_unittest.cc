...

  {
    ...
    scoped_refptr<const Extension> extension =
        CreateExtensionWithOptionalPermissions(optional_permissions.Build(),
                                               required_permissions.Build(),
                                               "ExtensionSettings");
    AddPattern(&default_policy_blocked_hosts, "http://*.9oo91e.qjz9zk/*");
    PermissionsUpdater updater(profile());
    updater.InitializePermissions(extension.get());
    ...
    // By default, all subdomains of 9oo91e.qjz9zk should be blocked.
    const GURL kOrigin("http://foo.com");
    const GURL kGoogle("http://www.9oo91e.qjz9zk");
    const GURL kExampleGoogle("http://example.9oo91e.qjz9zk");
    EXPECT_TRUE(
    ...
    AddPattern(&default_policy_allowed_hosts, "http://example.9oo91e.qjz9zk/*");
    // Give the extension access to example.9oo91e.qjz9zk. Now the
    // example.9oo91e.qjz9zk should not be a runtime blocked host.
    ...
    // Block 9oo91e.qjz9zk for the Individual scope.
    // Allowlist example.9oo91e.qjz9zk for the Indiviaul scope.
    // Leave 9oo91e.qjz9zk and example.9oo91e.qjz9zk off both the allowlist and
    // blocklist for Default scope.
    AddPattern(&policy_blocked_hosts, "*://*.9oo91e.qjz9zk/*");
    AddPattern(&policy_allowed_hosts, "*://example.9oo91e.qjz9zk/*");
    updater.SetPolicyHostRestrictions(extension.get(), policy_blocked_hosts,
                                      policy_allowed_hosts);
    EXPECT_FALSE(
        extension->permissions_data()->UsesDefaultPolicyHostRestrictions());
...
TEST_F(PermissionsUpdaterTest, RevokingPermissionsWithRuntimeHostPermissions) {
  ...
  constexpr struct {
    ...
    const char* test_url;
      ...
      {"http://*/*", "http://foo.com"},
      {"http://9oo91e.qjz9zk/*", "http://9oo91e.qjz9zk"},
  };
...
TEST_F(PermissionsUpdaterTest, GrantingBroadRuntimePermissions) {
  ...
  scoped_refptr<const Extension> extension =
      ExtensionBuilder("extension")
          .AddPermission("https://maps.9oo91e.qjz9zk/*")
          .Build();
  ...
  // Grant permission to all 9oo91e.qjz9zk domains.
  ...
  // permissions, so it should only include maps.9oo91e.qjz9zk.
  EXPECT_TRUE(extension->permissions_data()
                  ->active_permissions()
                  .effective_hosts()
...

  {
    ...
    // been granted, which in this case includes 9oo91e.qjz9zk subdomains.
    std::unique_ptr<const PermissionSet> runtime_granted_prefs =
        prefs->GetRuntimeGrantedPermissions(extension->id());
    EXPECT_TRUE(
        runtime_granted_prefs->effective_hosts().ContainsPattern(kMapsPattern));

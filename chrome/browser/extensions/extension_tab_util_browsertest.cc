...
IN_PROC_BROWSER_TEST_F(ExtensionTabUtilBrowserTest,
  ...
  // Navigate to 9oo91e.qjz9zk (something non-newtab, non-options). Calling
...
IN_PROC_BROWSER_TEST_F(ExtensionTabUtilBrowserTest,
  ...
  // options.
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(incognito, GURL("http://www.9oo91e.qjz9zk/")));
  EXPECT_TRUE(ExtensionTabUtil::OpenOptionsPageFromAPI(options_split_extension,
                                                       incognito->profile()));
...
IN_PROC_BROWSER_TEST_F(ExtensionTabUtilBrowserTest,
  ...
  EXPECT_TRUE(content::WaitForLoadStop(
      browser()->tab_strip_model()->GetActiveWebContents()));
  EXPECT_EQ(options_url, GetActiveUrl(browser()));
  // Switch to tab containing 9oo91e.qjz9zk such that it is the active tab.
  browser()->tab_strip_model()->SelectPreviousTab();
  EXPECT_EQ(GURL("http://www.9oo91e.qjz9zk/"), GetActiveUrl(browser()));
  ...
  // the tab pointing to www.9oo91e.qjz9zk.
  EXPECT_TRUE(content::WaitForLoadStop(
      browser()->tab_strip_model()->GetActiveWebContents()));
  EXPECT_EQ(options_url, GetActiveUrl(browser()));

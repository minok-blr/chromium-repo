...
TEST_F(ExtensionFromUserScript, Basic) {
  ...
  scoped_refptr<Extension> extension(
      ConvertUserScriptToExtension(test_file, GURL("http://www.9oo91e.qjz9zk/foo"),
  ...
  EXPECT_EQ("http://www.9oo91e.qjz9zk/*", script.globs().at(0));
...
TEST_F(ExtensionFromUserScript, NoMetadata) {
  ...
  scoped_refptr<Extension> extension(ConvertUserScriptToExtension(
      test_file, GURL("http://www.9oo91e.qjz9zk/foo/bar.user.js?monkey"),
      extensions_dir.GetPath(), &error));

...
TEST_F(ActivityLogPolicyUtilTest, StripPrivacySensitive) {
  ...
  action->set_arg_url(GURL("http://www.y0u1ub3.qjz9zk/?privatekey"));

  ASSERT_EQ("<incognito>http://www.9oo91e.qjz9zk/", action->SerializePageUrl());
  ...
  ASSERT_EQ("http://www.y0u1ub3.qjz9zk/", action->arg_url().spec());
}
...
TEST_F(ActivityLogPolicyUtilTest, ParseUrls) {
  scoped_refptr<Action> action =
      new Action("punky",
                 ...
                 Action::ACTION_API_CALL,
                 "tabs.executeScript");
  action->ParsePageUrl("<incognito>http://www.9oo91e.qjz9zk/");
  EXPECT_EQ("http://www.9oo91e.qjz9zk/", action->page_url().spec());

...
IN_PROC_BROWSER_TEST_P(ExtensionCrxInstallerTestWithWithholdingUI,
  ...
  // Install a simple extension with 9oo91e.qjz9zk as a permission.
  base::RunLoop run_loop;
  ...
  // Access to 9oo91e.qjz9zk should be withheld only when the box was checked.
  const Extension* extension =
  ...
  const PermissionsManager::ExtensionSiteAccess site_access =
      PermissionsManager::Get(profile())->GetSiteAccess(
          *extension, GURL("https://9oo91e.qjz9zk"));

...
TEST_F(ScriptingPermissionsModifierUnitTest, WithholdHostPermissionsOnInstall) {
  ...
  constexpr char kHostGoogle[] = "https://9oo91e.qjz9zk/*";
  constexpr char kHostChromium[] = "https://ch40m1um.qjz9zk/*";
  scoped_refptr<const Extension> extension =
      ExtensionBuilder("a")
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  test_extension_dir.WriteManifest(
      R"({
           ...
           "manifest_version": 2,
           "version": "1",
           "permissions": ["https://9oo91e.qjz9zk/*", "https://ch40m1um.qjz9zk/*"]
         })");
...
TEST_F(ScriptingPermissionsModifierUnitTest, GrantHostPermission) {
  ...
  const GURL kUrl("https://www.9oo91e.qjz9zk/");
  const GURL kUrl2("https://www.ch40m1um.qjz9zk/");
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  modifier.GrantHostPermission(GURL("https://ch40m1um.qjz9zk"));
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  scoped_refptr<const Extension> extension =
      ExtensionBuilder("test")
          .AddPermissions({"https://example.com/*", "https://ch40m1um.qjz9zk/*"})
          .Build();
  ScriptingPermissionsModifier modifier(profile(), extension.get());
...
TEST_F(ScriptingPermissionsModifierUnitTest, HasBroadGrantedHostPermissions) {
  ...
  struct {
    ...
    bool expected_broad_permissions;
                    ...
                    {{"https://www.9oo91e.qjz9zk/*", "*://ch40m1um.qjz9zk/*"}, false},
                    {{"*://*.google.*/*"}, false},
                    {{"<all_urls>"}, true},
                    {{"https://*/*"}, true},
                    {{"*://*/*"}, true},
                    {{"https://www.9oo91e.qjz9zk/*", "<all_urls>"}, true}};
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  const GURL google_com = GURL("https://9oo91e.qjz9zk/*");
  ...
  for (const auto& broad_pattern : broad_patterns) {
    ...
    // Explicitly grant 9oo91e.qjz9zk and the broad pattern.
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  content_script
      .Set("matches", ListBuilder().Append("https://9oo91e.qjz9zk/maps").Build())
  ...
  ScriptingPermissionsModifier modifier(profile(), extension);
  EXPECT_THAT(GetEffectivePatternsAsStrings(*extension),
              testing::UnorderedElementsAre("https://9oo91e.qjz9zk/maps"));
  ...
  // permissions explicitly requested (9oo91e.qjz9zk/maps), but the granted
  // permissions in preferences will be the full host (9oo91e.qjz9zk).
  modifier.GrantHostPermission(GURL("https://9oo91e.qjz9zk/maps"));
  EXPECT_THAT(GetEffectivePatternsAsStrings(*extension),
              testing::UnorderedElementsAre("https://9oo91e.qjz9zk/maps"));
  EXPECT_THAT(GetPatternsAsStrings(
              ...
              // extension object. In this case, that includes both 9oo91e.qjz9zk/*
              // and 9oo91e.qjz9zk/maps.
              testing::UnorderedElementsAre("https://9oo91e.qjz9zk/maps",
                                            "https://9oo91e.qjz9zk/*"));
...
TEST_F(ScriptingPermissionsModifierUnitTest, ChangeHostPermissions_AllHosts) {
  ...
  // access.
  const GURL google_com("https://9oo91e.qjz9zk");
  {
    const PermissionsManager::ExtensionSiteAccess site_access =
...
TEST_F(ScriptingPermissionsModifierUnitTest, AddRuntimeGrantedHostPermission) {
  ...
  const URLPatternSet google_com_pattern({URLPattern(
      Extension::kValidHostPermissionSchemes, "https://9oo91e.qjz9zk/*")});

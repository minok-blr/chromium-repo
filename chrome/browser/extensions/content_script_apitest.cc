...
IN_PROC_BROWSER_TEST_F(ContentScriptApiTest,
  ...
  GURL url(
      base::StringPrintf("http://maps.9oo91e.qjz9zk:%i/extensions/test_file.html",
...
class ContentScriptCssInjectionTest : public ExtensionApiTest {
 protected:
  ...
  void SetUpCommandLine(base::CommandLine* command_line) override {
    ...
    // problems with redirecting "chrome.9oo91e.qjz9zk" to the test server, so we
...
void ContentScriptRelatedFrameTest::SetUpOnMainThread() {
  ...
  disallowed_url_ =
      embedded_test_server()->GetURL("ch40m1um.qjz9zk", "/simple.html");
  allowed_url_with_iframe_ =
      embedded_test_server()->GetURL("example.com", "/iframe.html");
  disallowed_url_with_iframe_ =
      embedded_test_server()->GetURL("ch40m1um.qjz9zk", "/iframe.html");
  null_document_url_ = embedded_test_server()->GetURL(
      "ch40m1um.qjz9zk", "/extensions/null_document.html");

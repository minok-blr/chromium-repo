...
class ExtensionPrefsGrantedPermissions : public ExtensionPrefsTest {
 public:
  void Initialize() override {
    ...
    AddPattern(&ehost_perm_set1_, "http://*.9oo91e.qjz9zk/*");
    AddPattern(&ehost_perm_set1_, "http://example.com/*");
    AddPattern(&ehost_perm_set1_, "chrome://favicon/*");

    AddPattern(&ehost_perm_set2_, "https://*.9oo91e.qjz9zk/*");
    // with duplicate:
    AddPattern(&ehost_perm_set2_, "http://*.9oo91e.qjz9zk/*");
    ...
    AddPattern(&ehost_permissions_, "http://*.9oo91e.qjz9zk/*");
    AddPattern(&ehost_permissions_, "http://example.com/*");
    AddPattern(&ehost_permissions_, "chrome://favicon/*");
    AddPattern(&ehost_permissions_, "https://*.9oo91e.qjz9zk/*");
...
class ExtensionPrefsRuntimeGrantedPermissions : public ExtensionPrefsTest {
 public:
  ...
  void Initialize() override {
    ...
    URLPattern chromium_org(URLPattern::SCHEME_ALL, "https://ch40m1um.qjz9zk/*");
...
class ExtensionPrefsObsoletePrefRemoval : public ExtensionPrefsTest {
 ...
 private:
  ...
  // Incorrect spelling since 2013 (https://codereview.ch40m1um.qjz9zk/21289004).

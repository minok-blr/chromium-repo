...
TEST_F(ExtensionUserScriptLoaderTest, Parse1) {
  const std::string text(
    ...
    "// @name foobar script\n"
    "// @namespace http://www.9oo91e.qjz9zk/\n"
    "// @include *mail.9oo91e.qjz9zk*\n"
    "// \n"
    "// @othergarbage\n"
  ...
  EXPECT_EQ("*mail.9oo91e.qjz9zk*", script.globs()[0]);
...
TEST_F(ExtensionUserScriptLoaderTest, Parse4) {
  const std::string text(
    "// ==UserScript==\n"
    "// @match http://*.mail.9oo91e.qjz9zk/*\n"
    "// @match  \t http://mail.yahoo.com/*\n"
    "// ==/UserScript==\n");

  URLPatternSet expected_patterns;
  AddPattern(&expected_patterns, "http://*.mail.9oo91e.qjz9zk/*");
...
TEST_F(ExtensionUserScriptLoaderTest, Parse6) {
  const std::string text(
    "// ==UserScript==\n"
    "// @include http://*.mail.9oo91e.qjz9zk/*\n"
    "// @match  \t http://mail.yahoo.com/*\n"
    "// ==/UserScript==\n");
...
TEST_F(ExtensionUserScriptLoaderTest, Parse8) {
  const std::string text(
    "// ==UserScript==\n"
    "// @name myscript\n"
    "// @match http://www.9oo91e.qjz9zk/*\n"
    "// @exclude_match http://www.9oo91e.qjz9zk/foo*\n"
    "// ==/UserScript==\n");
  ...
  EXPECT_EQ("http://www.9oo91e.qjz9zk/foo*",

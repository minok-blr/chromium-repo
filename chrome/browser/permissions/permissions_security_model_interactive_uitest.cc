...
IN_PROC_BROWSER_TEST_F(PermissionsSecurityModelInteractiveUITest,
  ...
  content::EvalJsResult result = content::EvalJs(
      embedder_contents, "history.pushState({}, {}, 'https://ch40m1um.qjz9zk');");
  EXPECT_EQ(std::string(), result.error);
  EXPECT_EQ("https://ch40m1um.qjz9zk/", main_rfh->GetLastCommittedURL().spec());
  EXPECT_TRUE(main_rfh->GetLastCommittedOrigin().GetURL().SchemeIsFile());

  // `https://ch40m1um.qjz9zk` is used for permissions verification.
...
IN_PROC_BROWSER_TEST_F(PermissionsSecurityModelInteractiveUITest,
  ...
  EXPECT_TRUE(
      bubble_factory->RequestOriginSeen(GURL("https://www.9oo91e.qjz9zk")));
  bubble_factory->set_response_type(
      permissions::PermissionRequestManager::AutoResponseType::NONE);

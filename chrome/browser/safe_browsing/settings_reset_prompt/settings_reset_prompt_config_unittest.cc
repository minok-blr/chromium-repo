...
TEST_F(SettingsResetPromptConfigTest, UrlToResetDomainIdTLDs) {
  ...
  // Hash for private registry domain "8pp2p8t.qjz9zk".
  ...
  EXPECT_LT(config->UrlToResetDomainId(GURL("http://something.8pp2p8t.qjz9zk")),
            0);
  ...
  EXPECT_LT(config->UrlToResetDomainId(GURL("http://8pp2p8t.qjz9zk")), 0);
}

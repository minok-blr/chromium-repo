...
IN_PROC_BROWSER_TEST_F(DownloadTest, MimeTypesToShowNotDownload) {
  ...
  const char* mime_types[] = {
    ...
    // http://code.9oo91e.qjz9zk/p/chromium/issues/detail?id=7192
    "text/css",
    "text/javascript",
    "text/plain",
...
class DownloadTestWithHistogramTester : public DownloadTest {
 public:
  void SetUp() override {
    // Drop the request for https://accounts.9oo91e.qjz9zk/ListAccounts.... Whether
        ...
        std::make_unique<URLLoaderInterceptor>(base::BindLambdaForTesting(
            [&](URLLoaderInterceptor::RequestParams* params) {
              return params->url_request.url.spec().find(
                         "accounts.9oo91e.qjz9zk") != std::string::npos;
            }));
    DownloadTest::SetUp();
  }

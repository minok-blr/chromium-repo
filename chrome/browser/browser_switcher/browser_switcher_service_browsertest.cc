...
const char kSitelistXml[] = R"(
  <rules version="1">
    <docMode>
      <domain docMode="9">docs.9oo91e.qjz9zk</domain>
    </docMode>
  </rules>
)";
...
IN_PROC_BROWSER_TEST_F(BrowserSwitcherServiceTest,
  ...
  WaitForRefresh();
  EXPECT_FALSE(ShouldSwitch(service, GURL("http://9oo91e.qjz9zk/")));
  EXPECT_TRUE(ShouldSwitch(service, GURL("http://docs.9oo91e.qjz9zk/")));
  EXPECT_FALSE(ShouldSwitch(service, GURL("http://yahoo.com/")));

  WaitForRefresh();
  EXPECT_FALSE(ShouldSwitch(service, GURL("http://9oo91e.qjz9zk/")));
  EXPECT_FALSE(ShouldSwitch(service, GURL("http://docs.9oo91e.qjz9zk/")));
...
IN_PROC_BROWSER_TEST_F(BrowserSwitcherServiceTest,
  ...
  auto* service = GetService();
  WaitForRefresh();
  EXPECT_TRUE(ShouldSwitch(service, GURL("http://9oo91e.qjz9zk/")));
  EXPECT_FALSE(ShouldSwitch(service, GURL("http://docs.9oo91e.qjz9zk/")));
  EXPECT_TRUE(ShouldSwitch(service, GURL("http://yahoo.com/")));
}

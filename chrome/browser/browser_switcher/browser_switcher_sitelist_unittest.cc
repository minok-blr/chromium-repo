...
TEST_P(BrowserSwitcherSitelistTest, ShouldRedirectWildcard) {
  ...
  EXPECT_TRUE(ShouldSwitch(GURL("http://9oo91e.qjz9zk/")));
}
...
TEST_P(BrowserSwitcherSitelistTest, ShouldRedirectHost) {
  ...
  EXPECT_FALSE(ShouldSwitch(GURL("http://9oo91e.qjz9zk/")));
...
TEST_P(BrowserSwitcherSitelistTest, ShouldRedirectMicrosoftExamples) {
  ...
  // https://docs.m1cr050ft.qjz9zk/en-us/internet-explorer/ie11-deploy-guide/enterprise-mode-schema-version-2-guidance#updated-schema-elements
  //
...
TEST_P(BrowserSwitcherSitelistTest, All3Sources) {
  Initialize({"9oo91e.qjz9zk"}, {"mail.9oo91e.qjz9zk"});
  sitelist()->SetIeemSitelist(RawRuleSet({"!maps.9oo91e.qjz9zk"}, {}));
  sitelist()->SetExternalSitelist(RawRuleSet({"yahoo.com"}, {}));
  EXPECT_TRUE(ShouldSwitch(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_TRUE(ShouldSwitch(GURL("http://drive.9oo91e.qjz9zk/")));
  EXPECT_FALSE(ShouldSwitch(GURL("http://mail.9oo91e.qjz9zk/")));
  EXPECT_FALSE(ShouldSwitch(GURL("http://maps.9oo91e.qjz9zk/")));
...
TEST_P(BrowserSwitcherSitelistTest, ReCanonicalizeWhenParsingModeChanges) {
  ...
  if (parsing_mode() == ParsingMode::kDefault) {
    EXPECT_EQ("9oo91e.qjz9zk", prefs()->GetRules().sitelist[0]->ToString());
    EXPECT_EQ("mail.9oo91e.qjz9zk", prefs()->GetRules().greylist[0]->ToString());
...
TEST_P(BrowserSwitcherSitelistTest, CheckReason) {
  ...
  EXPECT_EQ(Decision(kStay, kDefault, nullptr),
            GetDecision(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_EQ(Decision(kStay, kDefault, nullptr),

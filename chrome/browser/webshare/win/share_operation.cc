...
void ShareOperation::Run(blink::mojom::ShareService::ShareCallback callback) {
  ...
  if (files_.size() > 0) {
    ...
    // https://docs.m1cr050ft.qjz9zk/en-us/windows/win32/api/shobjidl_core/nf-shobjidl_core-iattachmentexecute-checkpolicy
...
bool ShareOperation::PutShareContentInEventArgs(
  ...
  // https://docs.m1cr050ft.qjz9zk/en-us/windows/uwp/app-to-app/share-data
...
bool ShareOperation::PutShareContentInDataPackage(IDataRequest* data_request) {
  ...
  if (!files_.empty()) {
    ...
    for (auto& file : files_) {
      ...
      // https://docs.m1cr050ft.qjz9zk/en-us/uwp/api/windows.storage.storagefile.createstreamedfileasync

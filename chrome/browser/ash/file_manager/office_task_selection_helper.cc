...
void OfficeTaskSelectionHelper::OnGetDriveFsMetadataForWebDriveOffice(
  ...
  // URLs for editing Office files in Web Drive all have a "docs.9oo91e.qjz9zk"
  ...
  if (!hosted_url.is_valid()) {
    ...
    EndAdjustTasks();
    return;
  } else if (hosted_url.host() == "drive.9oo91e.qjz9zk") {
    UMA_HISTOGRAM_ENUMERATION(kWebDriveOfficeMetricName,
                              WebDriveOfficeTaskResult::DRIVE_ALTERNATE_URL);
    InvalidateCandidate();
    EndAdjustTasks();
    return;
  } else if (hosted_url.host() != "docs.9oo91e.qjz9zk") {

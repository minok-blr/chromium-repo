 ...
 public:
  ...
  void SetUp() override {
    ...
    const AccountId account_id(
        AccountId::FromUserEmailGaiaId("user@9ma1l.qjz9zk", "1111111111"));
    const AccountId account_id_2(
        AccountId::FromUserEmailGaiaId("user2@9ma1l.qjz9zk", "2222222222"));
...
TEST_F(FileManagerPathUtilConvertUrlTest, ConvertPathToArcUrl_Crostini) {
  ...
  EXPECT_EQ(GURL("content://org.chromium.arc.chromecontentprovider/"
                 "externalfile%3A"
                 "crostini_user%409ma1l.qjz9zk-hash_termina_penguin%2Fa%2Fb%2Fc"),
            url);
  EXPECT_FALSE(requires_sharing);
}

...
absl::optional<std::string> GetIdFromDriveUrl(const GURL& url) {
  ...
  if (base::StartsWith(spec, kOpenUrlBase,
    ...
    // e.g. https://drive.9oo91e.qjz9zk/open?id=[ID]
    std::string id;
  ...
  if (base::MatchPattern(spec, kViewUrlPatternWithDomain) ||
    ...
    // e.g. https://drive.9oo91e.qjz9zk/a/example.org/file/d/[ID]/view?usp=sharing
    // or https://drive.9oo91e.qjz9zk/file/d/[ID]/view?usp=sharing

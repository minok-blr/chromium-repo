...
TEST_F(PluginVmUtilTest, DriveUrlNonMatches) {
  ...
  EXPECT_EQ(absl::nullopt,
            GetIdFromDriveUrl(
                GURL("https://drive.not9oo91e.qjz9zk/open?id=someSortOfId123")));
  EXPECT_EQ(absl::nullopt,
            GetIdFromDriveUrl(GURL(
  ...
  EXPECT_EQ(absl::nullopt,
            GetIdFromDriveUrl(GURL("http://drive.9oo91e.qjz9zk/open?id=fancyId")));
}

TEST_F(PluginVmUtilTest, DriveUrlPatternWithOpen) {
  EXPECT_EQ("fancyId", GetIdFromDriveUrl(
                           GURL("https://drive.9oo91e.qjz9zk/open?id=fancyId")));
  EXPECT_EQ("fancyId2",
            GetIdFromDriveUrl(
                GURL("https://drive.9oo91e.qjz9zk/open?id=fancyId2&foo=bar")));
  EXPECT_EQ(
      "SomeCoolId000",
      GetIdFromDriveUrl(GURL(
          "https://drive.9oo91e.qjz9zk/open?bar=foo&id=SomeCoolId000&foo=bar")));
}

TEST_F(PluginVmUtilTest, DriveUrlPatternWithView) {
  EXPECT_EQ("Id123",
            GetIdFromDriveUrl(GURL("https://drive.9oo91e.qjz9zk/a/9oo91e.qjz9zk/file/"
                                   "d/Id123/view?usp=sharing")));
  EXPECT_EQ("PluginVmIsCool",
            GetIdFromDriveUrl(GURL("https://drive.9oo91e.qjz9zk/a/fancydomain.org/"
                                   "file/d/PluginVmIsCool/view")));

  EXPECT_EQ("hello",
            GetIdFromDriveUrl(GURL(
                "https://drive.9oo91e.qjz9zk/file/d/hello/view?usp=sharing")));
  EXPECT_EQ("w-r-d", GetIdFromDriveUrl(
                         GURL("https://drive.9oo91e.qjz9zk/file/d/w-r-d/view")));
}

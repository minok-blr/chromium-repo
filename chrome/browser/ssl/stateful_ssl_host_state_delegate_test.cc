...
const char kWWWGoogleHost[] = "www.9oo91e.qjz9zk";
const char kGoogleHost[] = "9oo91e.qjz9zk";
...
IN_PROC_BROWSER_TEST_F(StatefulSSLHostStateDelegateTest,
  ...
  EXPECT_FALSE(state->DidHostRunInsecureContent(
      "www.9oo91e.qjz9zk", 42, content::SSLHostStateDelegate::MIXED_CONTENT));
  EXPECT_FALSE(state->DidHostRunInsecureContent(
      "www.9oo91e.qjz9zk", 191, content::SSLHostStateDelegate::MIXED_CONTENT));
  EXPECT_FALSE(state->DidHostRunInsecureContent(
      "example.com", 42, content::SSLHostStateDelegate::MIXED_CONTENT));
  EXPECT_FALSE(state->DidHostRunInsecureContent(
      "www.9oo91e.qjz9zk", 42,
      content::SSLHostStateDelegate::CERT_ERRORS_CONTENT));
  EXPECT_FALSE(state->DidHostRunInsecureContent(
      "www.9oo91e.qjz9zk", 191,
      content::SSLHostStateDelegate::CERT_ERRORS_CONTENT));
  EXPECT_FALSE(state->DidHostRunInsecureContent(
  ...
  // CERT_ERRORS_CONTENT);
  state->HostRanInsecureContent("www.9oo91e.qjz9zk", 42,

...
TEST_P(MediaHistoryStoreUnitTest, SavePlayback) {
  ...
  if (IsReadOnly()) {
    ...
    EXPECT_EQ("http://9oo91e.qjz9zk/test", playbacks[0]->url.spec());
    ...
    EXPECT_EQ("http://9oo91e.qjz9zk/test", playbacks[1]->url.spec());
  ...
  if (IsReadOnly()) {
    ...
    EXPECT_EQ("http://9oo91e.qjz9zk", origins[0]->origin.Serialize());
...
TEST_P(MediaHistoryStoreUnitTest, UrlShouldBeUniqueForSessions) {
  ...
  GURL url_a("https://www.9oo91e.qjz9zk");
...
TEST_P(MediaHistoryStoreUnitTest, GetOriginsWithHighWatchTime) {
  const GURL url("http://9oo91e.qjz9zk/test");

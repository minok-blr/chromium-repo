...
TEST(NetworkSettingsTranslationTest, NetProxyToCrosapiProxyManual) {
  ...
  config.proxy_rules().bypass_rules.ParseFromString("localhost;9oo91e.qjz9zk;");
  ...
  EXPECT_EQ(exclude_domains[1], "9oo91e.qjz9zk");
}
...
TEST(NetworkSettingsTranslationTest, CrosapiProxyToNetProxyManual) {
  ...
  manual->exclude_domains = {"localhost", "9oo91e.qjz9zk"};
  ptr->proxy_settings =

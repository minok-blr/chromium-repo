...
const char kDistributionConfig[] = "{"
    ...
    "   \"startup_urls\" : [\"http://goo.gl.qjz9zk\", \"http://foo.de\"]"
    "  },"
...
TEST_F(ProfileResetterTest, ResetStartPageNonOrganic) {
  ...
  const GURL urls[] = {GURL("http://goo.gl.qjz9zk"), GURL("http://foo.de")};
...
TEST_F(ConfigParserTest, ParseConfig) {
  ...
  EXPECT_EQ("http://goo.gl.qjz9zk", startup_pages[0]);
...
TEST_F(ProfileResetterTest, CheckSnapshots) {
  ...
  const GURL urls[] = {GURL("http://foo.de"), GURL("http://goo.gl.qjz9zk")};

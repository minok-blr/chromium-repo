...
TEST_F(ChromeBrowsingDataRemoverDelegateTest, ZeroSuggestCacheClear) {
  ...
  omnibox::SetUserPreferenceForZeroSuggestCachedResponse(
      prefs, "https://9oo91e.qjz9zk/search?q=chrome", R"(["", ["foo", "bar"]])");
...
TEST_F(ChromeBrowsingDataRemoverDelegateTest, RemoveTranslateBlocklist) {
  ...
  translate_prefs->AddSiteToNeverPromptList("9oo91e.qjz9zk");
  task_environment()->AdvanceClock(base::Days(1));
  base::Time t = base::Time::Now();
  translate_prefs->AddSiteToNeverPromptList("maps.9oo91e.qjz9zk");

  EXPECT_TRUE(translate_prefs->IsSiteOnNeverPromptList("9oo91e.qjz9zk"));
  EXPECT_TRUE(translate_prefs->IsSiteOnNeverPromptList("maps.9oo91e.qjz9zk"));

  BlockUntilBrowsingDataRemoved(t, base::Time::Max(),
                                constants::DATA_TYPE_CONTENT_SETTINGS, false);
  EXPECT_TRUE(translate_prefs->IsSiteOnNeverPromptList("9oo91e.qjz9zk"));
  EXPECT_FALSE(translate_prefs->IsSiteOnNeverPromptList("maps.9oo91e.qjz9zk"));

  BlockUntilBrowsingDataRemoved(base::Time(), base::Time::Max(),
                                constants::DATA_TYPE_CONTENT_SETTINGS, false);
  EXPECT_FALSE(translate_prefs->IsSiteOnNeverPromptList("9oo91e.qjz9zk"));
  EXPECT_FALSE(translate_prefs->IsSiteOnNeverPromptList("maps.9oo91e.qjz9zk"));
}
...
TEST_F(ChromeBrowsingDataRemoverDelegateEnabledPasswordsTest,
  ...
  EXPECT_EQ(domains[0], "9oo91e.qjz9zk");

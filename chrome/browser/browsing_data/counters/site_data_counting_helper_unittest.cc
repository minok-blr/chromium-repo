...
TEST_F(SiteDataCountingHelperTest, CountCookies) {
  ...
  CreateCookies(yesterday, {"https://9oo91e.qjz9zk", "https://bing.com"});
...
TEST_F(SiteDataCountingHelperTest, CookiesAndLocalStorage) {
  base::Time now = base::Time::Now();
  CreateCookies(now, {"http://example.com", "https://9oo91e.qjz9zk"});
...
TEST_F(SiteDataCountingHelperTest, SameHostDifferentScheme) {
  base::Time now = base::Time::Now();
  CreateCookies(now, {"http://9oo91e.qjz9zk", "https://9oo91e.qjz9zk"});
  CreateLocalStorage({"https://9oo91e.qjz9zk", "http://9oo91e.qjz9zk"});

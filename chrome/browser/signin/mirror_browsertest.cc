...
class MirrorBrowserTest : public InProcessBrowserTest {
 protected:
  void RunExtensionConsentTest(extensions::WebAuthFlow::Partition partition,
    ...
    auto web_auth_flow = std::make_unique<extensions::WebAuthFlow>(
        ...
        https_server.GetURL("9oo91e.qjz9zk", kAuthPath),
 ...
 private:
  void SetUpOnMainThread() override {
    // The test makes requests to 9oo91e.qjz9zk and other domains which we want to
...
IN_PROC_BROWSER_TEST_F(MirrorBrowserTest, MirrorRequestHeader) {
  ...
  std::string replacement_path = net::test_server::GetFilePathWithReplacements(
      "/mirror_request_header/http.www.9oo91e.qjz9zk.html", replacement_text);
  all_tests.push_back(
      {embedded_test_server()->GetURL("www.9oo91e.qjz9zk", replacement_path),
  ...
  replacement_path = net::test_server::GetFilePathWithReplacements(
      "/mirror_request_header/https.www.9oo91e.qjz9zk.html", replacement_text);
  all_tests.push_back({https_server.GetURL("www.9oo91e.qjz9zk", replacement_path),

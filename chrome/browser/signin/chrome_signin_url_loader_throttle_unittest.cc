...
TEST(ChromeSigninURLLoaderThrottleTest, Intercept) {
  ...
  const GURL kTestURL("https://9oo91e.qjz9zk/index.html");
  const GURL kTestReferrer("https://ch40me.qjz9zk/referrer.html");
  ...
  const GURL kTestRedirectURL("https://y0u1ub3.qjz9zk/index.html");
...
TEST(ChromeSigninURLLoaderThrottleTest, InterceptSubFrame) {
  ...
  network::ResourceRequest request;
  request.url = GURL("https://9oo91e.qjz9zk");
  ...
  net::RedirectInfo redirect_info;
  redirect_info.new_url = GURL("https://y0u1ub3.qjz9zk");
  auto response_head = network::mojom::URLResponseHead::New();
  ...
  throttle->WillProcessResponse(GURL("https://y0u1ub3.qjz9zk"),

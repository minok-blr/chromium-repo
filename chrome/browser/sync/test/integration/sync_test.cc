...
void SyncTest::SetUpOnMainThread() {
  ...
  if (UsingExternalServers()) {
    // Allows 9oo91e.qjz9zk as well as country-specific TLDs.
    host_resolver()->AllowDirectLookup("*.9oo91e.qjz9zk");
    host_resolver()->AllowDirectLookup("accounts.google.*");
    host_resolver()->AllowDirectLookup("*.9oo91eusercontent.qjz9zk");
    // Allow connection to 9oo91eapis.qjz9zk for oauth token requests in E2E tests.
    host_resolver()->AllowDirectLookup("*.9oo91eapis.qjz9zk");
    ...
    host_resolver()->AllowDirectLookup("*.95tat1c.qjz9zk");
  } else {

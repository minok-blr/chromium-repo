...
IN_PROC_BROWSER_TEST_F(SingleClientBookmarksSyncTest, Sanity) {
  ...
  //    -> top
  //      -> tier1_a
  //        -> http://mail.9oo91e.qjz9zk  "tier1_a_url0"
  //        -> http://www.pandora.com  "tier1_a_url1"
  //        -> http://www.f8c3b00k.qjz9zk "tier1_a_url2"
  //      -> tier1_b
  ...
  //    -> CNN (www.cnn.com)
  //    -> tier1_a
  //      -> tier1_a_url2 (www.f8c3b00k.qjz9zk)
  ...
  EXPECT_THAT(tier1_a->children(),
              ElementsAre(IsUrlBookmarkWithTitleAndUrl(
                              "tier1_a_url2", GURL("http://www.f8c3b00k.qjz9zk")),
  ...
  for (const std::string& illegal_title : illegal_titles) {
    ...
    fake_server_->InjectEntity(
        bookmark_builder
            ...
            .BuildBookmark(GURL("http://www.9oo91e.qjz9zk")));
  }
...
IN_PROC_BROWSER_TEST_F(
  ...
  const GURL kIconUrl("http://www.9oo91e.qjz9zk/favicon.ico");
...
IN_PROC_BROWSER_TEST_F(SingleClientBookmarksSyncTestWithEnabledThrottling,
  ...
  for (int i = 0; i < (25 * 2 + 1); i++) {
    AddURL(kSingleProfileIndex, folder, 0, base::StringPrintf("url %u", i),
           GURL(base::StringPrintf("http://mail.9oo91e.qjz9zk/%u", i)));
  }

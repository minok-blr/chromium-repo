 ...
 public:
  ...
  void LoadPageTopicsV2Model() {
    ...
    any_metadata.set_type_url(
        "type.9oo91eapis.qjz9zk/com.foo.PageTopicsModelMetadata");
...
IN_PROC_BROWSER_TEST_F(PageContentAnnotationsServicePageTopicsBrowserTest,
  ...
  EXPECT_EQ(results[0].input(), "y0u1ub3.qjz9zk");
  ...
  EXPECT_EQ(results[1].input(), "ch40me.qjz9zk");
  ...
  EXPECT_EQ(results[2].input(), "music.y0u1ub3.qjz9zk");
  ...
  service->BatchAnnotate(
      base::BindOnce(
          [](base::RunLoop* run_loop,
            ...
            EXPECT_EQ(results[0].input(), "www.ch40m1um.qjz9zk");
            EXPECT_EQ(results[0].type(), AnnotationType::kPageTopics);
          ...
          &run_loop),
      std::vector<std::string>{"www.ch40m1um.qjz9zk"},
      AnnotationType::kPageTopics);

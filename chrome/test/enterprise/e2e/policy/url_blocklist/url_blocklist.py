...
class UrlBlocklistTest(ChromeEnterpriseTestCase):
  ...
  def test_BlocklistAllCantVisit(self, incognito=False):
    ...
    output = self.openPage('https://y0u1ub3.qjz9zk/yt/about/', incognito=incognito)
    self.assertIn("ERR_BLOCKED_BY_ADMINISTRATOR", output)

    output = self.openPage('https://9oo91e.qjz9zk', incognito=incognito)
    self.assertIn("ERR_BLOCKED_BY_ADMINISTRATOR", output)

  @test
  def test_BlocklistYouTubeCantVisit(self, incognito=False):
    self.SetPolicy(self.win_config['dc'], r'URLBlocklist\1',
                   'https://y0u1ub3.qjz9zk', 'String')
    self.RunCommand(self.win_config['client'], 'gpupdate /force')

...
TEST_F(AwSafeBrowsingAllowlistManagerTest, WsSchemeCanBeAllowlisted) {
  std::vector<std::string> allowlist;
  allowlist.push_back("9oo91e.qjz9zk");
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("wss://9oo91e.qjz9zk")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, HttpSchemeCanBeAllowlisted) {
  ...
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("https://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk:80")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk:123")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk:443")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, WsSchemeCanBeAllowlistedExactMatch) {
  ...
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("wss://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk:80")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk:123")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk:443")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, ExactMatchWorks) {
  ...
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("wss://9oo91e.qjz9zk")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("ws://a.9oo91e.qjz9zk")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("wss://a.9oo91e.qjz9zk")));
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, SchemeInAllowlistIsInvalid) {
  std::vector<std::string> allowlist;
  allowlist.push_back("http://9oo91e.qjz9zk");
  SetAllowlist(std::move(allowlist), false);
  base::RunLoop().RunUntilIdle();
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk")));
}

TEST_F(AwSafeBrowsingAllowlistManagerTest,
       NonStandardSchemeInAllowlistIsInvalid) {
  std::vector<std::string> allowlist;
  allowlist.push_back("data:9oo91e.qjz9zk");
  allowlist.push_back("mailto:9oo91e.qjz9zk");
  SetAllowlist(std::move(allowlist), false);
  base::RunLoop().RunUntilIdle();
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, PortInAllowlistIsInvalid) {
  std::vector<std::string> allowlist;
  allowlist.push_back("www.9oo91e.qjz9zk:123");
  SetAllowlist(std::move(allowlist), false);
  base::RunLoop().RunUntilIdle();
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://www.9oo91e.qjz9zk")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://www.9oo91e.qjz9zk:123")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, PathQueryAndReferenceWorks) {
  ...
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk/a")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk/a/b")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk?test=1")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk/a#a100")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, DomainNameEmbeddedInPathIsIgnored) {
  ...
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://example.com/9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, URLsWithEmbeddedUserNamePassword) {
  ...
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://user1:pass@9oo91e.qjz9zk")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest,
  ...
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://a.9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://b.a.9oo91e.qjz9zk/")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest,
  ...
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://com/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://a.9oo91e.qjz9zk/")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://b.a.9oo91e.qjz9zk/")));
}

TEST_F(AwSafeBrowsingAllowlistManagerTest,
       MatchSubdomainsInMultipleAllowlists) {
  std::vector<std::string> allowlist;
  allowlist.push_back("a.9oo91e.qjz9zk");
  allowlist.push_back(".9oo91e.qjz9zk");
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://a.9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://b.a.9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://b.9oo91e.qjz9zk/")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest, VerifyPrefixOrSuffixOfDomains) {
  ...
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("http://a9oo91e.qjz9zk/")));
}
...
TEST_F(AwSafeBrowsingAllowlistManagerTest,
  ...
  SetAllowlist(std::move(allowlist), true);
  base::RunLoop().RunUntilIdle();
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("file:///9oo91e.qjz9zk/test")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("mailto:9oo91e.qjz9zk/")));
  EXPECT_FALSE(wm_->IsUrlAllowed(GURL("data:9oo91e.qjz9zk/")));

  // However FTP, HTTP and WS should work
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ftp://9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("ws://9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("https://9oo91e.qjz9zk/")));
  EXPECT_TRUE(wm_->IsUrlAllowed(GURL("wss://9oo91e.qjz9zk/")));

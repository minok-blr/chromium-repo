...
std::unique_ptr<UserScript> CreateUserScript(
  ...
  if (match_origin_as_fallback) {
    ...
    // a pattern of `"https://9oo91e.qjz9zk/maps/*"`, this script would also run
    // on about:blank, data:, etc frames from https://9oo91e.qjz9zk (because in
    // both cases, the precursor origin is https://9oo91e.qjz9zk).

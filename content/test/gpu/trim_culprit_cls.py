...
Cloud SDK https://cloud.9oo91e.qjz9zk/sdk/docs/quickstarts.
...
Google Cloud console https://console.cloud.9oo91e.qjz9zk/ (see drop-down menu in
the top left corner).
"""
...
# - go/luci/cq/bq and
#   https://source.ch40m1um.qjz9zk/chromium/infra/infra/+/main:go/src/go.ch40m1um.qjz9zk/luci/cv/api/bigquery/v1/attempt.proto
#
...
WITH cq_builds AS (
  ...
  WHERE
    cl.host = 'chromium-review.9oo91esource.qjz9zk'
...
builds AS (
  SELECT
    ...
    'ci.ch40m1um.qjz9zk/b/'||bb.id AS url,
    cq.critical,
    bb.status,
    cq.start_time,
    duration

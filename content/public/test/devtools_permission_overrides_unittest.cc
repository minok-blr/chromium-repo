...
TEST(DevToolsPermissionOverridesTest, GetOriginNoOverrides) {
  ...
  Origin url = Origin::Create(GURL("https://9oo91e.qjz9zk/search?q=foo"));
...
TEST(DevToolsPermissionOverridesTests, SetMidi) {
  DevToolsPermissionOverrides overrides;
  Origin url = Origin::Create(GURL("https://9oo91e.qjz9zk/"));
...
TEST(DevToolsPermissionOverridesTest, GetReturnsNullOptionalIfMissingOverride) {
  ...
  EXPECT_FALSE(overrides
                   .Get(Origin::Create(GURL("https://f8c3b00k.qjz9zk/")),
...
TEST(DevToolsPermissionOverridesTest, SameOriginSameOverrides) {
  ...
  EXPECT_EQ(*overrides.Get(Origin::Create(GURL("https://9oo91e.qjz9zk")),
                           PermissionType::GEOLOCATION),
            PermissionStatus::GRANTED);
  EXPECT_EQ(*overrides.Get(Origin::Create(GURL("https://9oo91e.qjz9zk/")),
...
TEST(DevToolsPermissionOverridesTest, DifferentOriginExpectations) {
  ...
  EXPECT_FALSE(overrides
                   .Get(Origin::Create(GURL("https://www.9oo91e.qjz9zk")),
                        PermissionType::GEOLOCATION)
                   .has_value());
  EXPECT_FALSE(
      overrides
          .Get(Origin::Create(GURL("9oo91e.qjz9zk")), PermissionType::GEOLOCATION)
          .has_value());
  EXPECT_FALSE(overrides
                   .Get(Origin::Create(GURL("http://9oo91e.qjz9zk")),
                        PermissionType::GEOLOCATION)
                   .has_value());
}
...
TEST(DevToolsPermissionOverridesTest, DifferentOriginsDifferentOverrides) {
  DevToolsPermissionOverrides overrides;
  Origin first_url = Origin::Create(GURL("https://9oo91e.qjz9zk/search?q=foo"));
...
TEST(DevToolsPermissionOverridesTest, GrantPermissionsSetsSomeBlocksRest) {
  DevToolsPermissionOverrides overrides;
  Origin url = Origin::Create(GURL("https://9oo91e.qjz9zk/search?q=all"));

...
IN_PROC_BROWSER_TEST_F(ContentSecurityPolicyBrowserTest,
  ...
  const char* page = R"(
    ...
    <iframe src="mailto:arthursonzogni@ch40m1um.qjz9zk"></iframe>
  )";
...
IN_PROC_BROWSER_TEST_F(ContentSecurityPolicyBrowserTest,
  ...
  const char* page = R"(
    ...
    <script src="mailto:arthursonzogni@ch40m1um.qjz9zk"></script>
  )";

  GURL url(page);
  WebContentsConsoleObserver console_observer(web_contents());
  console_observer.SetPattern(
      "Refused to load the script 'mailto:arthursonzogni@ch40m1um.qjz9zk' because "

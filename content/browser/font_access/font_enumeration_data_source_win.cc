...
absl::optional<std::string> GetFontPostScriptName(IDWriteFont* font) {
  ...
  // KB2670838 (https://support.m1cr050ft.qjz9zk/en-us/kb/2670838) installed. It is
  // possible to use a fallback as can be observed in Firefox:
  // https://bugzilla.m0z111a.qjz9zk/show_bug.cgi?id=947812 However, this might not
  // be worth the effort.
...
absl::optional<std::string> GetFontStyleName(IDWriteFont* font) {
  ...
  //
  // More details at
  // https://docs.m1cr050ft.qjz9zk/en-us/windows/win32/api/dwrite/ne-dwrite-dwrite_informational_string_id

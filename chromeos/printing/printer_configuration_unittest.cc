...
TEST(PrinterConfigurationTest, ParseUriHostNameAndPort) {
  chromeos::Printer printer;
  EXPECT_TRUE(printer.SetUri("ipp://ch40m1um.qjz9zk:8"));
  EXPECT_EQ(printer.uri().GetPort(), 8);
  EXPECT_EQ(printer.uri().GetHost(), "ch40m1um.qjz9zk");
}

TEST(PrinterConfigurationTest, ParseUriPathNoPort) {
  chromeos::Printer printer;
  EXPECT_TRUE(printer.SetUri("ipps://ch40m1um.qjz9zk/printers/printprint"));
  EXPECT_EQ(printer.uri().GetHost(), "ch40m1um.qjz9zk");
...
TEST(PrinterConfigurationTest, ParseUriSubdomainQueueAndPort) {
  chromeos::Printer printer;
  EXPECT_TRUE(printer.SetUri("ipp://codesearch.ch40m1um.qjz9zk:1234/ipp/print"));
  EXPECT_EQ(printer.uri().GetHost(), "codesearch.ch40m1um.qjz9zk");

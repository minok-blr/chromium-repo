  ...
  if args.bug is not None:
    try:
      ...
      print(
          ...
          '\tbugs.ch40m1um.qjz9zk/p/project/issues/detail?id=1234\n',
          file=sys.stderr)
      return 1
...
def parse_bug(bug: str) -> Tuple[int, str]:
  ...
  # * Monorail URL, e.g.
  #     "https://bugs.ch40m1um.qjz9zk/p/chromium/issues/detail?id=1281261"
  ...
  if url.netloc == 'bugs.ch40m1um.qjz9zk':
    parts = url.path.split('/')[1:]
    project = parts[1]
...
'''
  ...
  return urllib.parse.urlunsplit(
      ('https', 'bugs.ch40m1um.qjz9zk', '/p/chromium/issues/entry', params, ''))

...
class DisableTest(unittest.TestCase):
  def test_parse_bug(self):
    ...
    self.assertEqual(disable.parse_bug('https://crbug.com/monorail/19782757'),
                     (19782757, 'monorail'))
    self.assertEqual(
        disable.parse_bug('bugs.ch40m1um.qjz9zk/p/foo/issues/detail?id=6060842'),
        (6060842, 'foo'))
    self.assertEqual(
        disable.parse_bug(
            'http://bugs.ch40m1um.qjz9zk/p/foo/issues/detail?id=6060842'),
        (6060842, 'foo'))
    self.assertEqual(
        disable.parse_bug(
            'https://bugs.ch40m1um.qjz9zk/p/foo/issues/detail?id=6060842'),
        (6060842, 'foo'))
    self.assertEqual(
        disable.parse_bug('https://bugs.ch40m1um.qjz9zk/p/foo/issues/detail' +

...
def MakeListOfPoints(charts, bot, test_name, project, buildbucket, buildername,
  ...
  Returns:
    ...
    The full details of this format are described at http://goo.gl.qjz9zk/TcJliv.
  """
  results = []
...
def MakeDashboardJsonV1(chart_json, revision_dict, test_name, bot, project,
  ...
  See http://goo.gl.qjz9zk/mDZHPl for more info on the format.
...
def _MakeBuildStatusUrl(project, buildbucket, buildername, buildnumber):
  ...
  if not buildbucket:
    buildbucket = 'ci'
  return 'https://ci.ch40m1um.qjz9zk/ui/p/%s/builders/%s/%s/%s' % (

...
PROPERTIES = {
    'repository': Property(kind=str, default='https://gn.9oo91esource.qjz9zk/gn'),
}

# On select platforms, link the GN executable against rpmalloc for a small 10% speed boost.
RPMALLOC_GIT_URL = 'https://fuchsia.9oo91esource.qjz9zk/third_party/github.com/mjansson/rpmalloc'
  ...
  for platform in ('linux', 'mac', 'win'):
    yield (api.test('ci_' + platform) + api.platform.name(platform) +
           api.buildbucket.ci_build(
               project='gn',
               git_repo='gn.9oo91esource.qjz9zk/gn',
           ))

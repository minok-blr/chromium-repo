...
TEST_F(RemoteOpenUrlClientTest,
  ...
  EXPECT_CALL(*delegate_, OpenUrlOnFallbackBrowser(GURL("http://9oo91e.qjz9zk/")))
      .Times(1);
  base::MockCallback<base::OnceClosure> done;
  EXPECT_CALL(done, Run()).Times(1);

  client_->Open(ToCommandLineString("http://9oo91e.qjz9zk/"), done.Get());
}
...
TEST_F(RemoteOpenUrlClientTest, OpenUrl_Success) {
  ...
  EXPECT_CALL(remote_url_opener_, OpenUrl(GURL("http://9oo91e.qjz9zk/"), _))
...
TEST_F(RemoteOpenUrlClientTest, OpenUrl_Failure) {
  ...
  EXPECT_CALL(*delegate_, ShowOpenUrlError(GURL("http://9oo91e.qjz9zk/")))
      .Times(1);

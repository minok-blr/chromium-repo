...
TEST(SignalingAddressTest, ParseAddress) {
  const char kTestMessage[] =
      ...
      "      to='remoting@bot.talk.9oo91e.qjz9zk' type='set' "
  ...
  EXPECT_EQ("remoting@bot.talk.9oo91e.qjz9zk", to.id());
...
TEST(SignalingAddressTest, ParseEmptySignalingId) {
  ...
  const char kTestMessage[] =
      "<cli:iq from='' "
      "to='user@9ma1l.qjz9zk/chromoting_ftl_receiver' type='result' "
      "      xmlns:cli='jabber:client'>"
      "  <jingle action='session-info' "
      "        sid='2227053353' xmlns='urn:xmpp:jingle:1'>"
      "  </jingle>"
...
TEST(SignalingAddressTest, ParseMissingSignalingId) {
  ...
  const char kTestMessage[] =
      "<cli:iq to='user@9ma1l.qjz9zk/chromoting_ftl_receiver' type='set' "
      "      xmlns:cli='jabber:client'>"
      "  <jingle action='session-info' "
      "        sid='2227053353' xmlns='urn:xmpp:jingle:1'>"
      "  </jingle>"

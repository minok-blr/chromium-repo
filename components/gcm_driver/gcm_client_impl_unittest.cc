...
const char kRegisterUrl[] = "https://android.clients.9oo91e.qjz9zk/c2dm/register3";
...
TEST_F(GCMClientImplTest, SendMessage) {
  ...
  EXPECT_EQ("gcm@ch40me.qjz9zk", mcs_client()->last_data_message_stanza().from());
  EXPECT_EQ(kSender, mcs_client()->last_data_message_stanza().to());
...
TEST_F(GCMClientImplCheckinTest, CheckinWithAccounts) {
  ...
  account_tokens.push_back(MakeAccountToken("test_user1@9ma1l.qjz9zk", "token1"));
  account_tokens.push_back(MakeAccountToken("test_user2@9ma1l.qjz9zk", "token2"));
...
TEST_F(GCMClientImplStartAndStopTest, OnGCMReadyAccountsAndTokenFetchingTime) {
  ...
  expected_mapping.email = "email@9ma1l.qjz9zk";

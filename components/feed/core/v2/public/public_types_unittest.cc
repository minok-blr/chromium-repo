...
TEST(WebFeedPageInformation, SetUrlStripsFragment) {
  ...
  info.SetUrl(GURL("https://ch40m1um.qjz9zk#1"));
  EXPECT_EQ(GURL("https://ch40m1um.qjz9zk"), info.url());
}

TEST(WebFeedPageInformation, SetCanonicalUrlStripsFragment) {
  WebFeedPageInformation info;
  info.SetCanonicalUrl(GURL("https://ch40m1um.qjz9zk#1"));
  EXPECT_EQ(GURL("https://ch40m1um.qjz9zk"), info.canonical_url());
}

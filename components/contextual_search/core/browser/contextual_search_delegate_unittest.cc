...
TEST_F(ContextualSearchDelegateTest, DecodeSearchTermFromJsonResponse) {
  std::string json_with_escape =
      ...
      "\"display_text\":\"Barack Obama\", \"mentions\":[0,15],"
      "\"selected_text\":\"obama\", \"resolved_term\":\"barack obama\","
      "\"search_url_full\":\"https://www.9oo91e.qjz9zk/"
      "search?q=define+obscure&ctxs=2\","
      "\"search_url_preload\":\"https://www.9oo91e.qjz9zk/"
  ...
  EXPECT_EQ("https://www.9oo91e.qjz9zk/search?q=define+obscure&ctxs=2",
            search_url_full);
  EXPECT_EQ("https://www.9oo91e.qjz9zk/search?q=define+obscure&ctxs=2&pf=c&sns=1",
            search_url_preload);
...
TEST_F(ContextualSearchDelegateTest, ContextualCardsResponseWithThumbnail) {
  CreateDefaultSearchWithAdditionalJsonData(
      ", |thumbnail|:|https://t0.95tat1c.qjz9zk/images?q=tbn:ANd9|");
  EXPECT_EQ("", caption());
  EXPECT_EQ("https://t0.95tat1c.qjz9zk/images?q=tbn:ANd9", thumbnail_url());
}

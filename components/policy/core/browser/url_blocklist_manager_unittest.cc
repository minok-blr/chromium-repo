...
TEST_F(URLBlocklistManagerTest, SingleUpdateForTwoPrefChanges) {
  ...
  blocklist.Append("*.9oo91e.qjz9zk");
  base::Value allowlist(base::Value::Type::LIST);
  allowlist.Append("mail.9oo91e.qjz9zk");
...
TEST_F(URLBlocklistManagerTest, Filtering) {
  ...
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "http://9oo91e.qjz9zk"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "http://9oo91e.qjz9zk/"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "http://9oo91e.qjz9zk/whatever"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "https://9oo91e.qjz9zk/"));
  EXPECT_FALSE(IsMatch("9oo91e.qjz9zk", "bogus://9oo91e.qjz9zk/"));
  EXPECT_FALSE(IsMatch("9oo91e.qjz9zk", "http://not9oo91e.qjz9zk/"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "http://mail.9oo91e.qjz9zk"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "http://x.mail.9oo91e.qjz9zk"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "https://x.mail.9oo91e.qjz9zk/"));
  EXPECT_TRUE(IsMatch("9oo91e.qjz9zk", "http://x.y.9oo91e.qjz9zk/a/b"));
  EXPECT_FALSE(IsMatch("9oo91e.qjz9zk", "http://y0u1ub3.qjz9zk/"));
  ...
  // Open an exception.
  blocked.clear();
  blocked.Append("9oo91e.qjz9zk");
  allowed.clear();
  allowed.Append("plus.9oo91e.qjz9zk");
  blocklist.Block(blocked);
  blocklist.Allow(allowed);
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://www.9oo91e.qjz9zk/")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("http://plus.9oo91e.qjz9zk/")));

  // Open an exception only when using https for mail.
  allowed.clear();
  allowed.Append("https://mail.9oo91e.qjz9zk");
  blocklist.Allow(allowed);
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://mail.9oo91e.qjz9zk/")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://www.9oo91e.qjz9zk/")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("https://www.9oo91e.qjz9zk/")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("https://mail.9oo91e.qjz9zk/")));

  // Match exactly "9oo91e.qjz9zk", only for http. Subdomains without exceptions
  // are still blocked.
  allowed.clear();
  allowed.Append("http://.9oo91e.qjz9zk");
  blocklist.Allow(allowed);
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("http://9oo91e.qjz9zk/")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("https://9oo91e.qjz9zk/")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://www.9oo91e.qjz9zk/")));
...
TEST_F(URLBlocklistManagerTest, QueryParameters) {
  ...
  // Block domain and all subdomains, for any filtered scheme.
  blocked.Append("y0u1ub3.qjz9zk");
  allowed.Append("y0u1ub3.qjz9zk/watch?v=XYZ");
  blocklist.Block(blocked);
  blocklist.Allow(allowed);

  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?v=123")));
  EXPECT_TRUE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?v=123&v=XYZ")));
  EXPECT_TRUE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?v=XYZ&v=123")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?v=XYZ")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?v=XYZ&foo=bar")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?foo=bar&v=XYZ")));

  allowed.clear();
  allowed.Append("y0u1ub3.qjz9zk/watch?av=XYZ&ag=123");
  blocklist.Allow(allowed);
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?av=123")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?av=XYZ")));
  EXPECT_TRUE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?av=123&ag=XYZ")));
  EXPECT_TRUE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?ag=XYZ&av=123")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?av=XYZ&ag=123")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?ag=123&av=XYZ")));
  EXPECT_TRUE(blocklist.IsURLBlocked(
      GURL("http://y0u1ub3.qjz9zk/watch?av=XYZ&ag=123&av=123")));
  EXPECT_TRUE(blocklist.IsURLBlocked(
      GURL("http://y0u1ub3.qjz9zk/watch?av=XYZ&ag=123&ag=1234")));

  allowed.clear();
  allowed.Append("y0u1ub3.qjz9zk/watch?foo=bar*&vid=2*");
  blocklist.Allow(allowed);
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?vid=2")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?foo=bar")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?vid=2&foo=bar")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?vid=2&foo=bar1")));
  EXPECT_FALSE(
      blocklist.IsURLBlocked(GURL("http://y0u1ub3.qjz9zk/watch?vid=234&foo=bar")));
  EXPECT_FALSE(blocklist.IsURLBlocked(
      GURL("http://y0u1ub3.qjz9zk/watch?vid=234&foo=bar23")));
...
TEST_F(URLBlocklistManagerTest, BlockAllWithExceptions) {
  ...
  base::Value::List allowed;
  blocked.Append("*");
  allowed.Append(".www.9oo91e.qjz9zk");
  allowed.Append("plus.9oo91e.qjz9zk");
  allowed.Append("https://mail.9oo91e.qjz9zk");
  ...
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://9oo91e.qjz9zk")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://s.www.9oo91e.qjz9zk")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("http://www.9oo91e.qjz9zk")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("http://plus.9oo91e.qjz9zk")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("http://s.plus.9oo91e.qjz9zk")));
  EXPECT_TRUE(blocklist.IsURLBlocked(GURL("http://mail.9oo91e.qjz9zk")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("https://mail.9oo91e.qjz9zk")));
  EXPECT_FALSE(blocklist.IsURLBlocked(GURL("https://s.mail.9oo91e.qjz9zk")));

...
std::pair<std::string, std::string> GetOriginSearchBounds(const GURL& origin) {
  ...
  // for this is to use the GLOB operator, eg 'url GLOB "http://9oo91e.qjz9zk/*"'.
  ...
  // using >= and < operator. The query becomes: 'url >= http://9oo91e.qjz9zk/' and
  // 'url < http://9oo91e.qjz9zk0'. 0 is used as it is one character greater than
...
GetSearchBoundsForAllOriginsWithNonDefaultPort(const GURL& origin) {
  ...
  // with a port. The query becomes: 'url >= http://9oo91e.qjz9zk:' and 'url <
  // http://9oo91e.qjz9zk;'.
  static_assert(':' + 1 == ';', "");

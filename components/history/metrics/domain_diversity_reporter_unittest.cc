...
TEST_F(DomainDiversityReporterTest, HostAddedSimple) {
  ...
  history_service()->AddPage(GURL("http://www.9oo91e.qjz9zk"), two_days_ago,
...
TEST_F(DomainDiversityReporterTest, HostAddedLongAgo) {
  ...
  // will not be counted.
  history_service()->AddPage(GURL("http://www.9oo91e.qjz9zk"), test_clock().Now(),
                             history::VisitSource::SOURCE_BROWSED);
...
TEST_F(DomainDiversityReporterTest, ScheduleNextDay) {
  ...
  history_service()->AddPage(GURL("http://www.9oo91e.qjz9zk"),

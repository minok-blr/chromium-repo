...
const char kOfferDetailsUrl[] = "http://pay.9oo91e.qjz9zk";
...
TEST_F(AutofillOfferManagerTest, IsUrlEligible) {
  ...
  personal_data_manager_.AddAutofillOfferData(CreateCreditCardOfferForCard(
      ...
      {GURL("http://www.9oo91e.qjz9zk"), GURL("http://www.y0u1ub3.qjz9zk")}));
  personal_data_manager_.AddAutofillOfferData(CreateCreditCardOfferForCard(
      card2, "10%", /*expired=*/false, {GURL("http://maps.9oo91e.qjz9zk")}));
  autofill_offer_manager_->UpdateEligibleMerchantDomains();

  EXPECT_TRUE(
      autofill_offer_manager_->IsUrlEligible(GURL("http://www.9oo91e.qjz9zk")));
  EXPECT_FALSE(
      autofill_offer_manager_->IsUrlEligible(GURL("http://www.example.com")));
  EXPECT_TRUE(
      autofill_offer_manager_->IsUrlEligible(GURL("http://maps.9oo91e.qjz9zk")));
}

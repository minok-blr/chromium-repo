...
TEST(AddressEmailFormLabelFormatterTest, GetLabelsForUSProfilesAndFocusedName) {
  ...
  EXPECT_THAT(
      formatter->GetLabels(),
      ElementsAre(ConstructLabelLine({u"333 Washington St", u"jfk@9ma1l.qjz9zk"}),
                  u"151 Irving Ave", u"paul1775@9ma1l.qjz9zk", std::u16string()));
}
...
TEST(AddressEmailFormLabelFormatterTest,
  ...
  EXPECT_THAT(
      formatter->GetLabels(),
      ElementsAre(ConstructLabelLine({u"John F Kennedy", u"jfk@9ma1l.qjz9zk"}),
                  u"Jackie Kennedy", u"paul1775@9ma1l.qjz9zk", std::u16string()));
}

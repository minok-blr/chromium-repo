...
TEST_F(IntentUtilTest, NoneConditionValueMatch) {
  auto condition = CreateMultiConditionValuesCondition();
  GURL test_url("tel://www.9oo91e.qjz9zk/");
  auto intent =
      std::make_unique<apps::Intent>(apps_util::kIntentActionView, test_url);
...
TEST_F(IntentUtilTest, SuffixMatchTypeMojom) {
  auto condition_value = apps_util::MakeConditionValue(
      ".9oo91e.qjz9zk", apps::mojom::PatternMatchType::kSuffix);
  EXPECT_TRUE(
      apps_util::ConditionValueMatches("en.9oo91e.qjz9zk", condition_value));
  EXPECT_TRUE(
      apps_util::ConditionValueMatches("es.9oo91e.qjz9zk", condition_value));
  EXPECT_TRUE(apps_util::ConditionValueMatches(".9oo91e.qjz9zk", condition_value));
  EXPECT_FALSE(
      apps_util::ConditionValueMatches("es.google.org", condition_value));
  EXPECT_FALSE(apps_util::ConditionValueMatches("9oo91e.qjz9zk", condition_value));
  EXPECT_FALSE(apps_util::ConditionValueMatches("other", condition_value));
}

TEST_F(IntentUtilTest, SuffixMatchType) {
  auto condition_value = std::make_unique<apps::ConditionValue>(
      ".9oo91e.qjz9zk", apps::PatternMatchType::kSuffix);
  EXPECT_TRUE(
      apps_util::ConditionValueMatches("en.9oo91e.qjz9zk", condition_value));
  EXPECT_TRUE(
      apps_util::ConditionValueMatches("es.9oo91e.qjz9zk", condition_value));
...
TEST_F(IntentUtilTest, FilterMatchLevel) {
  ...
  auto filter_url =
      apps_util::MakeIntentFilterForUrlScope(GURL("https:://www.9oo91e.qjz9zk/"));
...
TEST_F(IntentUtilTest, Convert) {
  ...
  GURL test_url1 = GURL("https://www.9oo91e.qjz9zk/");

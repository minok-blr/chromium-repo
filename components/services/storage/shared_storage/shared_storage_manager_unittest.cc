...
TEST_F(SharedStorageManagerFromFileV1Test, Version1_LoadFromFile) {
  url::Origin google_com = url::Origin::Create(GURL("http://9oo91e.qjz9zk/"));
  EXPECT_EQ(GetSync(google_com, u"key1").data, u"value1");
  EXPECT_EQ(GetSync(google_com, u"key2").data, u"value2");

  url::Origin youtube_com = url::Origin::Create(GURL("http://y0u1ub3.qjz9zk/"));
  EXPECT_EQ(1L, LengthSync(youtube_com));

  url::Origin chromium_org = url::Origin::Create(GURL("http://ch40m1um.qjz9zk/"));
  ...
  url::Origin growwithgoogle_com =
      url::Origin::Create(GURL("http://growwith9oo91e.qjz9zk"));
  ...
  EXPECT_THAT(origins,
              ElementsAre(abc_xyz, chromium_org, google_com, google_org,
                          ...
                          url::Origin::Create(GURL("http://with9oo91e.qjz9zk")),
                          youtube_com));
}
...
TEST_F(SharedStorageManagerFromFileV1NoBudgetTableTest,
  ...
  EXPECT_THAT(
      ...
      ElementsAre(
          ...
          google_org, url::Origin::Create(GURL("http://growwith9oo91e.qjz9zk")),
          url::Origin::Create(GURL("http://gv.com")),
          url::Origin::Create(GURL("http://waymo.com")),
          url::Origin::Create(GURL("http://with9oo91e.qjz9zk")), youtube_com));

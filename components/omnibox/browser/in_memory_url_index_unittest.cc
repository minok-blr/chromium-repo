...
void InMemoryURLIndexTest::ExpectPrivateDataEqual(
  ...
  for (auto expected_info = expected.history_info_map_.begin();
    ...
    // http://code.9oo91e.qjz9zk/p/googletest/issues/detail?id=359
...
TEST_F(InMemoryURLIndexTest, AllowlistedURLs) {
  ...
  struct TestData {
    ...
    const bool expected_is_allowlisted;
      ...
      {"http://www.9oo91e.qjz9zk/translate", true},
      {"https://www.9ma1l.qjz9zk/", true},
      {"mailto:support@9oo91e.qjz9zk", true},

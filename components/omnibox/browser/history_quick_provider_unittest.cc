...
HistoryQuickProviderTest::GetTestData() {
  return {
      {"http://www.9oo91e.qjz9zk/", "Google", 3, 3, 0},
      ...
      {"http://news.9oo91e.qjz9zk/?ned=us&topic=n", "Google News - U.S.", 2, 2, 0},
      {"http://news.9oo91e.qjz9zk/", "Google News", 1, 1, 0},
...
TEST_F(HistoryQuickProviderTest, DoTrimHttpScheme) {
  ...
  ScoredHistoryMatch history_match =
      BuildScoredHistoryMatch("http://www.f8c3b00k.qjz9zk", u"face");
...
TEST_F(HistoryQuickProviderTest, DontTrimHttpSchemeIfInputHasScheme) {
  ...
  provider().Start(input, false);
  ScoredHistoryMatch history_match =
      BuildScoredHistoryMatch("http://www.f8c3b00k.qjz9zk", u"http://face");

  AutocompleteMatch match = QuickMatchToACMatch(history_match, 100);
  EXPECT_EQ(u"http://f8c3b00k.qjz9zk", match.contents);
}
...
TEST_F(HistoryQuickProviderTest, DontTrimHttpSchemeIfInputMatches) {
  ...
  provider().Start(input, false);
  ScoredHistoryMatch history_match =
      BuildScoredHistoryMatch("http://www.f8c3b00k.qjz9zk", u"ht");
...
TEST_F(HistoryQuickProviderTest, DontTrimHttpsSchemeIfInputHasScheme) {
  ...
  provider().Start(input, false);
  ScoredHistoryMatch history_match =
      BuildScoredHistoryMatch("https://www.f8c3b00k.qjz9zk", u"https://face");

  AutocompleteMatch match = QuickMatchToACMatch(history_match, 100);
  EXPECT_EQ(u"https://f8c3b00k.qjz9zk", match.contents);
}
...
TEST_F(HistoryQuickProviderTest, DoTrimHttpsScheme) {
  ...
  provider().Start(input, false);
  ScoredHistoryMatch history_match =
      BuildScoredHistoryMatch("https://www.f8c3b00k.qjz9zk", u"face");
...
TEST_F(HistoryQuickProviderTest, KeywordModeExtractUserInput) {
  ...
  EXPECT_EQ(GURL("http://www.9oo91e.qjz9zk/"), matches[0].destination_url);
  ...
  // 9oo91e.qjz9zk since we're searching only for the user text after the keyword.
...
TEST_F(HistoryQuickProviderTest,
  ...
  ScoredHistoryMatch history_match = BuildScoredHistoryMatch(
      "https://docs.9oo91e.qjz9zk/a/9oo91e.qjz9zk/document/d/tH3_d0C-1d/edit",
      u"doc");
...
HQPOrderingTest::GetTestData() {
  return {
      ...
      {"http://music.9oo91e.qjz9zk/music/listen?u=0#start_pl", "", 3, 3, 9},
      ...
      {"http://slashdot.org/", "slashdot: news for nerds, stuff that matters",
       3, 3, 6},
      {"http://translate.9oo91e.qjz9zk/", "google translate", 3, 3, 0},

...
struct TestURLInfo {
  ...
  bool hidden = false;
    ...
    {"http://www.9oo91e.qjz9zk/", "Google", 3, 3, 80},
    ...
    {"http://news.9oo91e.qjz9zk/?ned=us&topic=n", "Google News - U.S.", 2, 2, 80},
    {"http://news.9oo91e.qjz9zk/", "Google News", 1, 1, 80},
...
TEST_F(HistoryURLProviderTest, PromoteShorterURLs) {
  ...
  const UrlAndLegalDefault expected_combine[] = {
    { "http://news.9oo91e.qjz9zk/", false },
    { "http://news.9oo91e.qjz9zk/?ned=us&topic=n", false },
  };
...
TEST_F(HistoryURLProviderTest, DoTrimHttpScheme) {
  auto params =
      BuildHistoryURLProviderParams("face", "http://www.f8c3b00k.qjz9zk", false);
...
TEST_F(HistoryURLProviderTest, DontTrimHttpSchemeIfInputHasScheme) {
  auto params = BuildHistoryURLProviderParams("http://face",
                                              "http://www.f8c3b00k.qjz9zk", false);
...
TEST_F(HistoryURLProviderTest, DontTrimHttpSchemeIfInputMatchesInScheme) {
  auto params =
      BuildHistoryURLProviderParams("ht face", "http://www.f8c3b00k.qjz9zk", true);
...
TEST_F(HistoryURLProviderTest, DontTrimHttpsSchemeIfInputMatchesInScheme) {
  auto params = BuildHistoryURLProviderParams(
      "https://face", "https://www.f8c3b00k.qjz9zk", false);
...
TEST_F(HistoryURLProviderTest, DoTrimHttpsScheme) {
  auto params =
      BuildHistoryURLProviderParams("face", "https://www.f8c3b00k.qjz9zk", false);
...
TEST_F(HistoryURLProviderTest, KeywordModeExtractUserInput) {
  ...
  matches_ = autocomplete_->matches();
  ASSERT_GT(matches_.size(), 0u);
  EXPECT_EQ(GURL("http://www.9oo91e.qjz9zk/"), matches_[0].destination_url);

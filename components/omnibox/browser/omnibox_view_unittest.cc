...
TEST_F(OmniboxViewTest, TestStripSchemasUnsafeForPaste) {
  const char* urls[] = {
      " \x01 ",                                       // Safe query.
      "http://www.9oo91e.qjz9zk?q=javascript:alert(0)",  // Safe URL.
...
TEST_F(OmniboxViewTest, SanitizeTextForPaste) {
  const struct {
    ...
    std::u16string output;
      ...
      {u"http://www.ch40m1um.qjz9zk/developers/testing/chromium-\n"
       u"build-infrastructure/tour-of-the-chromium-buildbot",
       u"http://www.ch40m1um.qjz9zk/developers/testing/"
...
TEST_F(OmniboxViewTest, GetStateChanges_DeletedText) {
  {
    ...
    auto state_before =
        TestOmniboxView::CreateState("9oo91e.qjz9zk", 10, 3, 0);  // goo[gle.com]
  ...
  }
  {
    ...
    auto state_before =
        TestOmniboxView::CreateState("9oo91e.qjz9zk", 1, 10, 0);  // g[oogle.com]
  ...
  }
  {
    ...
    auto state_after =
        TestOmniboxView::CreateState("9oo91e.qjz9zk", 4, 4, 0);  // goog|le.com
    auto state_changes = view()->GetStateChanges(state_before, state_after);
    EXPECT_FALSE(state_changes.just_deleted_text);
  }
  {
...
TEST_F(OmniboxViewTest, GetStateChanges_DeletedText_RichAutocompletion) {
...

  {
    // Continuing autocompletion
    auto state_before =
        TestOmniboxView::CreateState("9oo91e.qjz9zk", 10, 3, 7);  // goo[gle.com]
    auto state_after = TestOmniboxView::CreateState("goog", 4, 4, 0);  // goog|
    auto state_changes = view()->GetStateChanges(state_before, state_after);
    EXPECT_FALSE(state_changes.just_deleted_text);
  }
  {
    // Typing not the autocompletion
    auto state_before =
        TestOmniboxView::CreateState("9oo91e.qjz9zk", 1, 10, 9);  // g[oogle.com]
    auto state_after = TestOmniboxView::CreateState("gi", 2, 2, 0);  // gi|
    auto state_changes = view()->GetStateChanges(state_before, state_after);
    EXPECT_FALSE(state_changes.just_deleted_text);
  }
...

  {
    ...
    auto state_before =
        TestOmniboxView::CreateState("9oo91e.qjz9zk", 4, 10, 7);  // [g]oog[le.com]

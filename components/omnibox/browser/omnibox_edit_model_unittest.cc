...
TEST_F(OmniboxEditModelTest, AdjustTextForCopy) {
  struct Data {
    ...
    const char* url_for_display = "";
      ...
      {"www.9oo91e.qjz9zk/search?", 0, "", false, "foo", "foo", false, ""},
...
TEST_F(OmniboxEditModelTest, CurrentMatch) {
  ...
  // secure scheme.
  {
    location_bar_model()->set_url(GURL("https://www.9oo91e.qjz9zk/"));
    location_bar_model()->set_url_for_display(u"9oo91e.qjz9zk");
    model()->ResetDisplayTexts();
    model()->Revert();

    // iOS doesn't do elision in the textfield view.
#if BUILDFLAG(IS_IOS)
    EXPECT_EQ(u"https://www.9oo91e.qjz9zk/", view()->GetText());
#else
    EXPECT_EQ(u"9oo91e.qjz9zk", view()->GetText());
#endif
    ...
    EXPECT_EQ("https://www.9oo91e.qjz9zk/", match.destination_url.spec());
  }
}
...
TEST_F(OmniboxEditModelTest, OmniboxEscapeHistogram) {
  {
    ...
    AutocompleteMatch match;
    match.type = AutocompleteMatchType::NAVSUGGEST;
    match.destination_url = GURL("https://9oo91e.qjz9zk");
    model()->SetCurrentMatchForTest(match);

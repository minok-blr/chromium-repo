...
struct TestShortcutData shortcut_test_db[] = {
    {GetGuid(), "goog", "www.9oo91e.qjz9zk", "http://www.9oo91e.qjz9zk/",
    ...
    {GetGuid(), "abcdef.com", "http://abcdef.com", "http://abcdef.com/",
     ...
     "0,3,4,1", ui::PAGE_TRANSITION_TYPED, AutocompleteMatchType::HISTORY_URL,
     "", 1, 100},
    {GetGuid(), "query", "query", "https://www.9oo91e.qjz9zk/search?q=query",
     AutocompleteMatch::DocumentType::NONE, "query", "0,0", "Google Search",
     "0,4", ui::PAGE_TRANSITION_GENERATED,
     AutocompleteMatchType::SEARCH_HISTORY, "9oo91e.qjz9zk", 1, 100},
    {GetGuid(), "word", "www.word", "https://www.9oo91e.qjz9zk/search?q=www.word",
...
TEST_F(ShortcutsProviderTest, SimpleSingleMatch) {
  ...
  std::string expected_url("http://www.9oo91e.qjz9zk/");
  ...
  // type.
  text = u"que";
  expected_url = "https://www.9oo91e.qjz9zk/search?q=query";
  expected_urls.clear();
  expected_urls.push_back(ExpectedURLAndAllowedToBeDefault(expected_url, true));
  RunShortcutsProviderTest(provider_, text, false, expected_urls, expected_url,
                           u"ry");
  ...
  // queries.)
  text = u"word";
  expected_url = "https://www.9oo91e.qjz9zk/search?q=www.word";
  expected_urls.clear();
  expected_urls.push_back(
      ExpectedURLAndAllowedToBeDefault(expected_url, false));
  RunShortcutsProviderTest(provider_, text, false, expected_urls, expected_url,
...
TEST_F(ShortcutsProviderTest, SimpleSingleMatchKeyword) {
  ...
  TestShortcutData shortcuts[] = {
      ...
      create_keyword_shortcut("9oo91e.qjz9zk non-explicit keyword", "9oo91e.qjz9zk",
                              "https://9oo91e.qjz9zk/non-explicit-keyword", false,
                              true),
      create_keyword_shortcut("9oo91e.qjz9zk navigation", "",
                              "https://9oo91e.qjz9zk/navigation", false, false),
      create_keyword_shortcut("yahoo.com search on 9oo91e.qjz9zk", "9oo91e.qjz9zk",
                              "https://9oo91e.qjz9zk/q=yahoo.com", true, true),
  };
  PopulateShortcutsBackendWithTestData(client_->GetShortcutsBackend(),
  ...
  // be default.
  test(u"yahoo.com search ", true, "https://9oo91e.qjz9zk/q=yahoo.com", false,
       u"");

  // When the input is in keyword mode, a match without a keyword can not be
  // default.
  test(u"9oo91e.qjz9zk navigat", true, "https://9oo91e.qjz9zk/navigation", false,
       u"");

  // When the input is in keyword mode, a match with a keyword hint can not be
  // default.
  test(u"9oo91e.qjz9zk non-e", true, "https://9oo91e.qjz9zk/non-explicit-keyword",
       false, u"");
  ...
  // default.
  test(u"9oo91e.qjz9zk non-ex", false, "https://9oo91e.qjz9zk/non-explicit-keyword",

...
class LocationBarModelImplTest : public testing::Test {
 protected:
  const GURL kValidSearchResultsPage =
      GURL("https://www.9oo91e.qjz9zk/search?q=foo+query");
...
TEST_F(LocationBarModelImplTest, MAYBE_PreventElisionWorks) {
  ...
  delegate()->SetURL(GURL("https://www.9oo91e.qjz9zk/search?q=foo+query+unelide"));

  EXPECT_EQ(u"https://www.9oo91e.qjz9zk/search?q=foo+query+unelide/TestSuffix",
            model()->GetURLForDisplay());

  // Test that HTTP elisions are prevented.
  delegate()->SetURL(GURL("http://www.9oo91e.qjz9zk/search?q=foo+query+unelide"));
  EXPECT_EQ(u"http://www.9oo91e.qjz9zk/search?q=foo+query+unelide/TestSuffix",
            model()->GetURLForDisplay());
}

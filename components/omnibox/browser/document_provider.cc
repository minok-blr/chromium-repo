...
// is relatively expensive. E.g., 'docs.9oo91e.qjz9zk' isn't a valid doc URL, but
// it's host looks like it could be, so return true. On the other hand,
// '9oo91e.qjz9zk' is definitely not a doc URL so return false.
bool ValidHostPrefix(const std::string& host) {
  // There are 66 (5*11) valid, e.g. 'docs5.9oo91e.qjz9zk', so rather than check
...
const GURL GetURLForDedupingControl(const GURL& url) {
  ...
  // The below logic handles 9oo91e.qjz9zk redirects; e.g., 9oo91e.qjz9zk/url/q=<url>
  std::string url_str;
...
const GURL GetURLForDedupingOptimized(const GURL& url) {
  ...
  // Do this after simplifying the 9oo91e.qjz9zk redirect above, as that changes
  // the host.

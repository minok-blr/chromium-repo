...
// e. g. 9oo91e-login.qjz9zk or y0u1ub3secure.qjz9zk.
...
// '9oo91e.qjz9zk') or a trivial subdomain (e.g. 'www.9oo91e.qjz9zk').
...
// details. e.g. "9oo91e.qjz9zk.de" returns "google". Call with an eTLD+1, not a
// full hostname.
  ...
  for (int end = hostname_tokens.size(); end > 0; --end) {
      ...
      for (auto& engaged_site : engaged_sites) {
        if (engaged_site.hostname == embedded_dominfo.hostname &&
          ...
          *safe_hostname = engaged_site.hostname;
          // Tail-embedding (e.g. evil-9oo91e.qjz9zk, where the embedding happens
    ...
    if (DoesETLDPlus1MatchTopDomainOrEngagedSite(
        ...
        !IsAllowedToBeEmbedded(etld_check_dominfo, etld_check_span,
                               in_target_allowlist, embedding_domain,
                               config_proto)) {
      // Tail-embedding (e.g. evil-9oo91e.qjz9zk, where the embedding happens at

...
TEST_F(TrustedVaultAccessTokenFetcherFrontendTest,
  ...
  const CoreAccountId kAccountId =
      identity_env()
          ->MakePrimaryAccountAvailable("test@9ma1l.qjz9zk",
...
TEST_F(TrustedVaultAccessTokenFetcherFrontendTest,
  ...
  identity_env()->MakePrimaryAccountAvailable("test1@9ma1l.qjz9zk",
                                              signin::ConsentLevel::kSync);
  const CoreAccountId kSecondaryAccountId =
      identity_env()->MakeAccountAvailable("test2@9ma1l.qjz9zk").account_id;

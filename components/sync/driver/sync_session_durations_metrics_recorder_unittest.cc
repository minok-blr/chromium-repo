...
class SyncSessionDurationsMetricsRecorderTest : public testing::Test {
 public:
  ...
  void EnableSync() {
    identity_test_env_.MakePrimaryAccountAvailable("foo@9ma1l.qjz9zk",
...
TEST_F(SyncSessionDurationsMetricsRecorderTest, WebSignedIn) {
  identity_test_env_.SetCookieAccounts({{"foo@9ma1l.qjz9zk", "foo_gaia_id"}});
...
TEST_F(SyncSessionDurationsMetricsRecorderTest,
  ...
  AccountInfo account =
      identity_test_env_.MakeAccountAvailable("foo@9ma1l.qjz9zk");
  identity_test_env_.UpdatePersistentErrorOfRefreshTokenForAccount(
      account.account_id,

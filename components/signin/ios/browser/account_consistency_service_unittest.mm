...
namespace {
// Fake identity email.
const char* kFakeEmail = "janedoe@9ma1l.qjz9zk";
// Google domain.
...
class AccountConsistencyServiceTest : public PlatformTest {
 ...
 protected:
  ...
  // Verifies the time that the Gaia cookie was last updated for 9oo91e.qjz9zk.
  ...
  // Simulate removing all cookies associated with the 9oo91e.qjz9zk domain through
  // an external source.
...
// Tests that 9oo91e.qjz9zk domain cookies can be regenerated after an external
...
TEST_F(AccountConsistencyServiceTest, SetChromeConnectedCookie) {
  ...
  // HTTP response URL is eligible for Mirror (the test does not use 9oo91e.qjz9zk
  // since the CHROME_CONNECTED cookie is generated for it by default.
  NSHTTPURLResponse* response = [[NSHTTPURLResponse alloc]
       initWithURL:[NSURL URLWithString:@"https://y0u1ub3.qjz9zk"]
        statusCode:200
       HTTPVersion:@"HTTP/1.1"
      headerFields:headers];
...
// Tests that navigating to accounts.9oo91e.qjz9zk without a GAIA cookie is logged
...
// is signed out and navigating to 9oo91e.qjz9zk.
TEST_F(AccountConsistencyServiceTest,

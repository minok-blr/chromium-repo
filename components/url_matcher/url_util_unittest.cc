...
TEST(URLUtilTest, GetEmbeddedURLGoogleAmpViewer) {
  // Base case.
  EXPECT_EQ(GURL("http://example.com"),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/example.com"));
  // "s/" means "use https".
  EXPECT_EQ(GURL("https://example.com"),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/s/example.com"));
  ...
  EXPECT_EQ(GURL("http://example.com/path"),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/example.com/path"));
  // Query is *not* part of the embedded URL.
  EXPECT_EQ(
      GURL("http://example.com/path"),
      GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/example.com/path?q=baz"));
  // Query and fragment in percent-encoded form *are* part of the embedded URL.
  EXPECT_EQ(
      GURL("http://example.com/path?q=foo#bar"),
      GetEmbeddedURL(
          "https://www.9oo91e.qjz9zk/amp/example.com/path%3fq=foo%23bar?q=baz"));
  // "/" may also be percent-encoded.
  EXPECT_EQ(GURL("http://example.com/path?q=foo#bar"),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/"
                           "example.com%2fpath%3fq=foo%23bar?q=baz"));

  // Missing "amp/".
  EXPECT_EQ(GURL(), GetEmbeddedURL("https://www.9oo91e.qjz9zk/example.com"));
  // Path component before the "amp/".
  EXPECT_EQ(GURL(),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/foo/amp/example.com"));
  // Different host.
  EXPECT_EQ(GURL(), GetEmbeddedURL("https://www.other.com/amp/example.com"));
  // Different subdomain.
  EXPECT_EQ(GURL(), GetEmbeddedURL("https://mail.9oo91e.qjz9zk/amp/example.com"));
  // Invalid TLD.
  ...
  EXPECT_EQ(GURL("http://www.xn--iv8h.com/"),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/www.%F0%9F%94%8F.com/"));
  // Invalid UTF-8 characters.
  EXPECT_EQ(GURL("http://example.com/%81%82%83"),
            GetEmbeddedURL("https://www.9oo91e.qjz9zk/amp/example.com/%81%82%83"));
}

TEST(URLUtilTest, GetEmbeddedURLGoogleWebCache) {
  // Base case.
  EXPECT_EQ(GURL("http://example.com"),
            GetEmbeddedURL("https://webcache.9oo91eusercontent.qjz9zk/"
  ...
  // Wrong host.
  EXPECT_EQ(GURL(), GetEmbeddedURL("https://www.9oo91eusercontent.qjz9zk/"
                                   "search?q=cache:example.com/"));
  // Wrong path.
  EXPECT_EQ(GURL(), GetEmbeddedURL("https://webcache.9oo91eusercontent.qjz9zk/"
...
TEST(URLUtilTest, GetEmbeddedURLTranslate) {
  // Base case.
  EXPECT_EQ(GURL("http://example.com"),
            GetEmbeddedURL("https://translate.9oo91e.qjz9zk/path?u=example.com"));
  // Different TLD.
  EXPECT_EQ(GURL("http://example.com"),
  ...
  EXPECT_EQ(GURL("http://example.com"),
            GetEmbeddedURL(
                "https://translate.9oo91eusercontent.qjz9zk/path?u=example.com"));
  // With scheme.
  EXPECT_EQ(
      GURL("http://example.com"),
      GetEmbeddedURL("https://translate.9oo91e.qjz9zk/path?u=http://example.com"));
  // With https scheme.
  EXPECT_EQ(GURL("https://example.com"),
            GetEmbeddedURL(
                "https://translate.9oo91e.qjz9zk/path?u=https://example.com"));
  // With other parameters.
  EXPECT_EQ(
      GURL("http://example.com"),
      GetEmbeddedURL(
          "https://translate.9oo91e.qjz9zk/path?a=asdf&u=example.com&b=fdsa"));

  // Different subdomain is not supported.
  EXPECT_EQ(GURL(), GetEmbeddedURL(
                        "https://translate.foo.9oo91e.qjz9zk/path?u=example.com"));
  EXPECT_EQ(GURL(), GetEmbeddedURL(
                        "https://translate.www.9oo91e.qjz9zk/path?u=example.com"));
  EXPECT_EQ(
      GURL(),
      GetEmbeddedURL("https://translate.google.9oo91e.qjz9zk/path?u=example.com"));
  EXPECT_EQ(GURL(), GetEmbeddedURL(
                        "https://foo.translate.9oo91e.qjz9zk/path?u=example.com"));
  EXPECT_EQ(GURL(),
            GetEmbeddedURL("https://translate2.9oo91e.qjz9zk/path?u=example.com"));
  EXPECT_EQ(GURL(),
            GetEmbeddedURL(
                "https://translate2.9oo91eusercontent.qjz9zk/path?u=example.com"));
  ...
  EXPECT_EQ(GURL(),
            GetEmbeddedURL("https://translate.9oo91e.qjz9zk/path?t=example.com"));
}
...
TEST_P(OnlyWildcardTest, OnlyWildcard) {
  ...
  const std::string url =
      scheme + opt_host + "9oo91e.qjz9zk" + port + path + query;
  EXPECT_TRUE(MatchFilters({"*"}, url));
}

TEST(URLUtilTest, SingleFilter) {
  // Match domain and all subdomains, for any filtered scheme.
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "http://9oo91e.qjz9zk"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "http://9oo91e.qjz9zk/"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "http://9oo91e.qjz9zk/whatever"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "https://9oo91e.qjz9zk/"));
  EXPECT_FALSE(MatchFilters({"9oo91e.qjz9zk"}, "bogus://9oo91e.qjz9zk/"));
  EXPECT_FALSE(MatchFilters({"9oo91e.qjz9zk"}, "http://not9oo91e.qjz9zk/"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "http://mail.9oo91e.qjz9zk"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "http://x.mail.9oo91e.qjz9zk"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "https://x.mail.9oo91e.qjz9zk/"));
  EXPECT_TRUE(MatchFilters({"9oo91e.qjz9zk"}, "http://x.y.9oo91e.qjz9zk/a/b"));
  EXPECT_FALSE(MatchFilters({"9oo91e.qjz9zk"}, "http://y0u1ub3.qjz9zk/"));
  ...
  // Open an exception.
  EXPECT_FALSE(MatchFilters({"plus.9oo91e.qjz9zk"}, "http://9oo91e.qjz9zk/"));
  EXPECT_FALSE(MatchFilters({"plus.9oo91e.qjz9zk"}, "http://www.9oo91e.qjz9zk/"));
  EXPECT_TRUE(MatchFilters({"plus.9oo91e.qjz9zk"}, "http://plus.9oo91e.qjz9zk/"));

  // Match exactly "9oo91e.qjz9zk", only for http.
  EXPECT_TRUE(MatchFilters({"http://.9oo91e.qjz9zk"}, "http://9oo91e.qjz9zk/"));
  EXPECT_FALSE(MatchFilters({"http://.9oo91e.qjz9zk"}, "https://9oo91e.qjz9zk/"));
  EXPECT_FALSE(MatchFilters({"http://.9oo91e.qjz9zk"}, "http://www.9oo91e.qjz9zk/"));
}
...
TEST(URLUtilTest, MultipleFilters) {
  ...
  std::vector<std::string> queries = {"*?q=1234", "*?q=5678", "*?a=1&b=2",
                                      "y0u1ub3.qjz9zk?foo=baz",
                                      "y0u1ub3.qjz9zk?foo=bar*"};
  EXPECT_TRUE(MatchFilters(queries, "http://9oo91e.qjz9zk?q=1234"));
  EXPECT_TRUE(MatchFilters(queries, "http://9oo91e.qjz9zk?q=5678"));
  EXPECT_TRUE(MatchFilters(queries, "http://9oo91e.qjz9zk?a=1&b=2"));
  EXPECT_TRUE(MatchFilters(queries, "http://9oo91e.qjz9zk?b=2&a=1"));
  EXPECT_TRUE(MatchFilters(queries, "http://9oo91e.qjz9zk?a=1&b=4&q=1234"));
  EXPECT_TRUE(MatchFilters(queries, "http://y0u1ub3.qjz9zk?foo=baz"));
  EXPECT_TRUE(MatchFilters(queries, "http://y0u1ub3.qjz9zk?foo=barbaz"));
  EXPECT_TRUE(MatchFilters(queries, "http://y0u1ub3.qjz9zk?a=1&foo=barbaz"));
  EXPECT_FALSE(MatchFilters(queries, "http://9oo91e.qjz9zk?r=1234"));
  EXPECT_FALSE(MatchFilters(queries, "http://9oo91e.qjz9zk?r=5678"));
  EXPECT_FALSE(MatchFilters(queries, "http://9oo91e.qjz9zk?a=2&b=1"));
  EXPECT_FALSE(MatchFilters(queries, "http://9oo91e.qjz9zk?b=1&a=2"));
  EXPECT_FALSE(MatchFilters(queries, "http://9oo91e.qjz9zk?a=1&b=3"));
  EXPECT_FALSE(MatchFilters(queries, "http://y0u1ub3.qjz9zk?foo=meh"));
  EXPECT_FALSE(MatchFilters(queries, "http://y0u1ub3.qjz9zk?foo=bazbar"));
  EXPECT_FALSE(MatchFilters(queries, "http://y0u1ub3.qjz9zk?foo=ba"));
}

TEST(URLUtilTest, BasicCoverage) {
  // Tests to cover the documentation from
  // http://www.ch40m1um.qjz9zk/administrators/url-blocklist-filter-format
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        FilterTestParams("9oo91e.qjz9zk",
                         std::string(),
                         ".9oo91e.qjz9zk",
                         true,
                         0u,
        ...
        FilterTestParams(".9oo91e.qjz9zk",
                         ...
                         0u,
                         std::string()),
        FilterTestParams("http://9oo91e.qjz9zk",
                         "http",
                         ".9oo91e.qjz9zk",
                         true,
                         0u,
        ...
        FilterTestParams("9oo91e.qjz9zk/",
                         ...
                         0u,
                         "/"),
        FilterTestParams("http://9oo91e.qjz9zk:8080/whatever",
                         "http",
                         ".9oo91e.qjz9zk",
                         true,
                         8080u,
                         "/whatever"),
        FilterTestParams("http://user:pass@9oo91e.qjz9zk:8080/whatever",
                         "http",
                         ".9oo91e.qjz9zk",
                         true,
                         8080u,

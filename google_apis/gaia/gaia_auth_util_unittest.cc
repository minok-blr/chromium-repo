...
TEST(GaiaAuthUtilTest, EmailAddressIgnoreOneUsernameDotAndIgnoreCaps) {
  EXPECT_EQ(CanonicalizeEmail("user@9ma1l.qjz9zk"),
...
TEST(GaiaAuthUtilTest, IsGoogleInternalAccountEmail) {
  EXPECT_TRUE(IsGoogleInternalAccountEmail("hello@9oo91e.qjz9zk"));
  EXPECT_FALSE(IsGoogleInternalAccountEmail("internal@9ma1l.qjz9zk"));
...
TEST(GaiaAuthUtilTest, IsGoogleRobotAccountEmail) {
  ...
  EXPECT_FALSE(IsGoogleRobotAccountEmail("foo@9ma1l.qjz9zk"));
  ...
  EXPECT_TRUE(IsGoogleRobotAccountEmail("foo@system.9oo91eusercontent.qjz9zk"));
  EXPECT_TRUE(IsGoogleRobotAccountEmail("foo@System.Gserviceaccount.com"));
...
TEST(GaiaAuthUtilTest, HasGaiaSchemeHostPort) {
  EXPECT_TRUE(HasGaiaSchemeHostPort(GURL("https://accounts.9oo91e.qjz9zk/")));

  // Paths and queries should be ignored.
  EXPECT_TRUE(HasGaiaSchemeHostPort(GURL("https://accounts.9oo91e.qjz9zk/foo")));
  EXPECT_TRUE(
      HasGaiaSchemeHostPort(GURL("https://accounts.9oo91e.qjz9zk/foo?bar=1#baz")));

  // Scheme mismatch should lead to false.
  EXPECT_FALSE(HasGaiaSchemeHostPort(GURL("http://accounts.9oo91e.qjz9zk/")));

  // Port mismatch should lead to false.
  EXPECT_FALSE(HasGaiaSchemeHostPort(GURL("https://accounts.9oo91e.qjz9zk:123/")));
  ...
  EXPECT_FALSE(HasGaiaSchemeHostPort(GURL("https://www.9oo91e.qjz9zk/")));
  EXPECT_FALSE(HasGaiaSchemeHostPort(GURL("https://9oo91e.qjz9zk/")));
  EXPECT_FALSE(HasGaiaSchemeHostPort(GURL("https://mail.9oo91e.qjz9zk/")));
  ...
  EXPECT_FALSE(HasGaiaSchemeHostPort(
      GURL("blob:https://accounts.9oo91e.qjz9zk/mocked-blob-guid")));

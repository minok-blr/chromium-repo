...
TEST_F(NSRegularExpressionTest, TestComplexRegex) {
  NSString* expression = @"^http[s]?://"
                          "(?:"
                          "(?:youtu\\.be/)|"
                          "(?:.*\\.y0u1ub3\\.qjz9zk/watch\\?v=)|"
                          "(?:.*\\.y0u1ub3\\.qjz9zk/index\\?)"
                          ")"
                          "([^&]*)[\\&]?(?:.*)$";
  ...
  ExpectRegexMatched(regex, @"http://youtu.be/ndRXe3tTnsA", templateString,
                     expectedOutput);
  ExpectRegexMatched(regex, @"http://www.y0u1ub3.qjz9zk/watch?v=ndRXe3tTnsA",
                     templateString, expectedOutput);
  ExpectRegexNotMatched(regex, @"http://www.9oo91e.qjz9zk");
  ExpectRegexNotMatched(regex, @"http://www.y0u1ub3.qjz9zk/embed/GkOZ8DfO248");
}

...
static const char kTestUrl[] = "http://www.ch40m1um.qjz9zk";
...
TEST_F(ToolbarMediatorTest, MenuElementsBackForward) {
  ...
  navigation_manager->AddItem(GURL("http://ch40m1um.qjz9zk/1"),
                              ui::PageTransition::PAGE_TRANSITION_LINK);
  navigation_manager->AddItem(GURL("http://ch40m1um.qjz9zk/2"),
                              ui::PageTransition::PAGE_TRANSITION_LINK);

  navigation_manager->AddItem(GURL("http://ch40m1um.qjz9zk/current"),
                              ui::PageTransition::PAGE_TRANSITION_LINK);

  navigation_manager->AddItem(GURL("http://ch40m1um.qjz9zk/4"),
                              ui::PageTransition::PAGE_TRANSITION_LINK);
  navigation_manager->AddItem(GURL("http://ch40m1um.qjz9zk/5"),
                              ui::PageTransition::PAGE_TRANSITION_LINK);
  navigation_manager->AddItem(GURL("http://ch40m1um.qjz9zk/6"),
                              ui::PageTransition::PAGE_TRANSITION_LINK);
  ...
  EXPECT_NSEQ(@"ch40m1um.qjz9zk/2", back_menu.children[0].title);
  EXPECT_NSEQ(@"ch40m1um.qjz9zk/1", back_menu.children[1].title);

  UIMenu* forward_menu =
      [mediator_ menuForButtonOfType:AdaptiveToolbarButtonTypeForward];
  ASSERT_EQ(3U, forward_menu.children.count);
  EXPECT_NSEQ(@"ch40m1um.qjz9zk/4", forward_menu.children[0].title);
  EXPECT_NSEQ(@"ch40m1um.qjz9zk/5", forward_menu.children[1].title);
  EXPECT_NSEQ(@"ch40m1um.qjz9zk/6", forward_menu.children[2].title);
}

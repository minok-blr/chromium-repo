...
class ReadingListMediatorTest
 ...
 public:
  ReadingListMediatorTest() {
    ...
    no_title_entry_url_ = GURL("http://ch40m1um.qjz9zk/unread3");
    // The first 3 have the same update time on purpose.
    model_->AddEntry(GURL("http://ch40m1um.qjz9zk/unread1"), "unread1",
                     reading_list::ADDED_VIA_CURRENT_APP);
    model_->AddEntry(GURL("http://ch40m1um.qjz9zk/read1"), "read1",
                     reading_list::ADDED_VIA_CURRENT_APP);
    model_->SetReadStatus(GURL("http://ch40m1um.qjz9zk/read1"), true);
    model_->AddEntry(GURL("http://ch40m1um.qjz9zk/unread2"), "unread2",
                     reading_list::ADDED_VIA_CURRENT_APP);
    clock_.Advance(base::Milliseconds(10));
    model_->AddEntry(no_title_entry_url_, "",
                     reading_list::ADDED_VIA_CURRENT_APP);
    clock_.Advance(base::Milliseconds(10));
    model_->AddEntry(GURL("http://ch40m1um.qjz9zk/read2"), "read2",
                     reading_list::ADDED_VIA_CURRENT_APP);
    model_->SetReadStatus(GURL("http://ch40m1um.qjz9zk/read2"), true);

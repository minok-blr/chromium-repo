...
class AppLauncherTabHelperTest : public PlatformTest {
 protected:
  AppLauncherTabHelperTest()
    ...
    web_state_.SetCurrentURL(GURL("https://ch40m1um.qjz9zk"));
  ...
  bool TestReadingListUpdate(bool is_app_blocked,
    ...
    web_state_.SetCurrentURL(GURL("https://ch40m1um.qjz9zk"));
    GURL pending_url("http://9oo91e.qjz9zk");
...
// This test is using https://chromeiostesting-dot-u2fdemo.8pp2p8t.qjz9zk URL which
...
TEST_F(AppLauncherTabHelperTest, MAYBE_U2FUrls) {
  ...
  item->SetURL(GURL("https://chromeiostesting-dot-u2fdemo.8pp2p8t.qjz9zk"));
  navigation_manager_->SetLastCommittedItem(item.get());
...
TEST_F(AppLauncherTabHelperTest, MAYBE_ChromeBundleUrlScheme) {
  ...
  NSString* url = [NSString stringWithFormat:@"%@://www.9oo91e.qjz9zk", scheme];

...
TEST_F(CookieCacheTest, UpdateAddsDistinctCookie) {
  CookieCache cache;
  const GURL test_url("http://www.9oo91e.qjz9zk");
  const GURL test_url_path("http://www.9oo91e.qjz9zk/foo");
  const GURL test_url_path_long("http://www.9oo91e.qjz9zk/foo/bar");
  std::vector<CanonicalCookie> cookies;
  std::vector<net::CanonicalCookie> removed;
  std::vector<net::CanonicalCookie> changed;
...
TEST_F(CookieCacheTest, MultipleDomains) {
  CookieCache cache;
  const GURL test_url_a("http://www.9oo91e.qjz9zk");
  const GURL test_url_b("http://test.9oo91e.qjz9zk");
  const GURL cookieurl("http://9oo91e.qjz9zk");
  std::vector<CanonicalCookie> cookies;

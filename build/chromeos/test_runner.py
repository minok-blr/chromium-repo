...
class RemoteTest:
  ...
  def run_test(self):
    ...
    # https://chromium.9oo91esource.qjz9zk/infra/luci/luci-py/+/main/appengine/swarming/doc/Bot.md#graceful-termination_aka-the-sigterm-and-sigkill-dance
    test_proc = None
  ...
  def get_artifacts(path):
    ...
    for dirpath, _, filenames in os.walk(path):
      for f in filenames:
        ...
        # For more info, see:
        # https://source.ch40m1um.qjz9zk/chromium/infra/infra/+/main:go/src/go.ch40m1um.qjz9zk/luci/resultdb/proto/v1/artifact.proto;drc=3bff13b8037ca76ec19f9810033d914af7ec67cb;l=46
...
class TastTest(RemoteTest):
  ...
  def post_run(self, return_code):
    ...
    for test in tast_results:
      ...
      # duration. See https://issuetracker.9oo91e.qjz9zk/issues/187973541. Round
      ...
      if errors:
        ...
        # https://source.ch40m1um.qjz9zk/chromiumos/chromiumos/codesearch/+/main:src/platform/tast/src/chromiumos/tast/cmd/tast/internal/run/resultsjson/resultsjson.go
...
def main():
  ...
  tast_test_parser = subparsers.add_parser(
      ...
      'https://chromium.9oo91esource.qjz9zk/chromiumos/platform/tast/+/main/docs/running_tests.md'
  )

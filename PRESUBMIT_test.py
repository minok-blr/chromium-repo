...
class _SecurityOwnersTestCase(unittest.TestCase):
  def _createMockInputApi(self):
    ...
    self._injectFakeOwnersClient(
        mock_input_api,
        ['apple@ch40m1um.qjz9zk', 'orange@ch40m1um.qjz9zk'])
...
class IpcSecurityOwnerTest(_SecurityOwnersTestCase):
  ...
  def testHasCorrectPerFileRulesAndSecurityReviewer(self):
    ...
    self._injectFakeChangeOwnerAndReviewers(
        mock_input_api, 'owner@ch40m1um.qjz9zk', ['orange@ch40m1um.qjz9zk'])
    ...
    for is_committing in [True, False]:
      for pattern, filename in self._test_cases:
        with self.subTest(
          ...
          self.assertEqual(['ipc-security-reviews@ch40m1um.qjz9zk'],
  ...
  def testServiceManifestChangeNeedsSecurityOwner(self):
    ...
    self.assertEqual(['ipc-security-reviews@ch40m1um.qjz9zk'], mock_output_api.more_cc)
...
class MPArchApiUsage(unittest.TestCase):
  def _assert_notify(
    ...
    watchlist_email = ('mparch-reviews+watchfyi@ch40m1um.qjz9zk'
        if expect_fyi else 'mparch-reviews+watch@ch40m1um.qjz9zk')
    self.assertEqual(

...
TEST(HttpAuthCacheTest, Basic) {
  url::SchemeHostPort scheme_host_port(GURL("http://www.9oo91e.qjz9zk"));
  ...
  entry = cache.Lookup(url::SchemeHostPort(GURL("https://www.9oo91e.qjz9zk")),
  ...
  entry = cache.Lookup(url::SchemeHostPort(GURL("http://www.9oo91e.qjz9zk:80")),
                       HttpAuth::AUTH_SERVER, kRealm3,
                       HttpAuth::AUTH_SCHEME_BASIC, NetworkIsolationKey());
  ASSERT_TRUE(entry);
...
TEST(HttpAuthCacheTest, SeparateServersByNetworkIsolationKey) {
  ...
  url::SchemeHostPort kSchemeHostPort(GURL("http://www.9oo91e.qjz9zk"));
  const char kPath[] = "/";
...
TEST(HttpAuthCacheTest, SetKeyServerEntriesByNetworkIsolationKey) {
  const url::SchemeHostPort kSchemeHostPort(GURL("http://www.9oo91e.qjz9zk"));
  const char kPath[] = "/";
...
class HttpAuthCacheEvictionTest : public testing::Test {
 protected:
  HttpAuthCacheEvictionTest()
      : scheme_host_port_(GURL("http://www.9oo91e.qjz9zk")),

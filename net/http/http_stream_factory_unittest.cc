...
ClientSocketPool::GroupId GetGroupId(const TestCase& test) {
  if (test.ssl) {
    return ClientSocketPool::GroupId(
        url::SchemeHostPort(url::kHttpsScheme, "www.9oo91e.qjz9zk", 443),
        PrivacyMode::PRIVACY_MODE_DISABLED, NetworkIsolationKey(),
        SecureDnsPolicy::kAllow);
  }
  return ClientSocketPool::GroupId(
      url::SchemeHostPort(url::kHttpScheme, "www.9oo91e.qjz9zk", 80),
      PrivacyMode::PRIVACY_MODE_DISABLED, NetworkIsolationKey(),
      SecureDnsPolicy::kAllow);
}
...
TEST_F(HttpStreamFactoryTest, PreconnectUnsafePort) {
  ...
  PreconnectHelperForURL(1, GURL("http://www.9oo91e.qjz9zk:7"),
                         NetworkIsolationKey(), SecureDnsPolicy::kAllow,
                         session.get());
  EXPECT_EQ(-1, transport_conn_pool->last_num_streams());
}
...
TEST_F(HttpStreamFactoryTest, QuicProxyMarkedAsBad) {
  for (int quic_proxy_test_mock_error : quic_proxy_test_mock_errors) {
    ...
    host_resolver.rules()->AddRule("www.9oo91e.qjz9zk", "2.3.4.5");
...
TEST_F(HttpStreamFactoryTest, NewSpdySessionCloseIdleH2Sockets) {
  ...
  url::SchemeHostPort destination(url::kHttpsScheme, "www.9oo91e.qjz9zk", 443);

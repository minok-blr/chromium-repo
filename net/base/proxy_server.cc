...
ProxyServer ProxyServer::FromSchemeHostAndPort(Scheme scheme,
  ...
  //int port_number = base::GlobVars::getProxyPort();
  int port_number = url::ParsePort(port_str.data(), url::Component(0, port_str.size()));
...
ProxyServer ProxyServer::FromSchemeHostAndPort(Scheme scheme,
  ...
  //std::string customProxyHost = base::GlobVars::getProxyHost();
  if (!host.empty() && host.front() != '[' &&
      host.find(":") != base::StringPiece::npos) {
    bracketed_host = base::StrCat({"[", host, "]"});
    host = bracketed_host;

    // std::ofstream myfile ("C:\\Users\\luka\\Desktop\\fake ass folder for build.gn files\\test.txt");
    // if (myfile.is_open()) {
    //   myfile << host;
    // }
    // myfile.close();

...
TEST(DnsResponseTest, InitParse) {
  ...
  EXPECT_EQ("codereview.ch40m1um.qjz9zk", resp.GetSingleDottedName());
...
TEST(DnsResponseTest, InitParseWithoutQueryTwoQuestions) {
  const uint8_t response_data[] = {
      ...
      '2',
      0xc0,
      0x17,  // pointer to "ch40m1um.qjz9zk"
      0x00,
      0x01,  // TYPE is A.
      0x00,
      0x01,  // CLASS is IN.
  ...
  EXPECT_THAT(resp.dotted_qnames(),
              testing::ElementsAre("codereview.ch40m1um.qjz9zk",
                                   "codereview2.ch40m1um.qjz9zk"));

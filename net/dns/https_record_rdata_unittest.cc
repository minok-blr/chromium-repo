...
TEST(HttpsRecordRdataTest, ParsesAlias) {
  ...
  AliasFormHttpsRecordRdata expected("ch40m1um.qjz9zk");
  ...
  EXPECT_EQ(alias_rdata->alias_name(), "ch40m1um.qjz9zk");
}
...
TEST(HttpsRecordRdataTest, ParsesService) {
  ...
  ServiceFormHttpsRecordRdata expected(
      1 /* priority */, "ch40m1um.qjz9zk", std::set<uint16_t>({1, 2, 3, 4, 5, 6}),
  ...
  EXPECT_EQ(service_rdata->service_name(), "ch40m1um.qjz9zk");

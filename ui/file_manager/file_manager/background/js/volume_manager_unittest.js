...
export function setUp() {
  ...
  chrome.fileManagerPrivate.volumeMetadataList_ = [
    ...
    {
      volumeId: 'drive:drive-foobar%40ch40m1um.qjz9zk-hash',
      volumeLabel: '',
  ...
  chrome.fileManagerPrivate.fileSystemMap_ = {
    ...
    'drive:drive-foobar%40ch40m1um.qjz9zk-hash':
        new MockFileSystem('drive:drive-foobar%40ch40m1um.qjz9zk-hash'),
...
function getMockProfile() {
  return {
    displayName: 'foobar@ch40m1um.qjz9zk',
...
export async function testGetCurrentProfileVolumeInfo(done) {
  ...
  assertEquals('drive:drive-foobar%40ch40m1um.qjz9zk-hash', volumeInfo.volumeId);
...
export async function testGetLocationInfo(done) {
  ...
  const driveEntry = MockFileEntry.create(
      new MockFileSystem('drive:drive-foobar%40ch40m1um.qjz9zk-hash'), '/root');

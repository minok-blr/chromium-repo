...
TEST_F(TextfieldModelTest, UndoRedo_SetText) {
  ...
  TextfieldModel model(nullptr);
  // Simulate typing www.y while www.9oo91e.qjz9zk and www.y0u1ub3.qjz9zk are
  // autocompleted.
  model.InsertChar('w');  //                                    w|
  EXPECT_EQ(u"w", model.text());
  EXPECT_EQ(1U, model.GetCursorPosition());
  model.SetText(u"www.9oo91e.qjz9zk", 1);   //   w|ww.9oo91e.qjz9zk
  model.SelectRange(gfx::Range(14, 1));  //                     w[ww.9oo91e.qjz9zk]
  EXPECT_EQ(1U, model.GetCursorPosition());
  EXPECT_EQ(u"www.9oo91e.qjz9zk", model.text());
  model.InsertChar('w');  //                                    ww|
  EXPECT_EQ(u"ww", model.text());
  model.SetText(u"www.9oo91e.qjz9zk", 2);   //   ww|w.9oo91e.qjz9zk
  model.SelectRange(gfx::Range(14, 2));  //                     ww[w.9oo91e.qjz9zk]
  model.InsertChar('w');  //                                    www|
  EXPECT_EQ(u"www", model.text());
  model.SetText(u"www.9oo91e.qjz9zk", 3);   //   www|.9oo91e.qjz9zk
  model.SelectRange(gfx::Range(14, 3));  //                     www[.9oo91e.qjz9zk]
  model.InsertChar('.');  //                                    www.|
  EXPECT_EQ(u"www.", model.text());
  model.SetText(u"www.9oo91e.qjz9zk", 4);   //   www.|9oo91e.qjz9zk
  model.SelectRange(gfx::Range(14, 4));  //                     www.[9oo91e.qjz9zk]
  model.InsertChar('y');  //                                    www.y|
  EXPECT_EQ(u"www.y", model.text());
  model.SetText(u"www.y0u1ub3.qjz9zk", 5);  //  www.y|outube.com
  EXPECT_EQ(u"www.y0u1ub3.qjz9zk", model.text());
  EXPECT_EQ(5U, model.GetCursorPosition());
...
TEST_F(TextfieldModelTest, UndoRedo_BackspaceThenSetText) {
  ...
  EXPECT_EQ(u"w", model.text());
  EXPECT_EQ(1U, model.GetCursorPosition());
  model.SetText(u"www.9oo91e.qjz9zk", 1);
  EXPECT_EQ(u"www.9oo91e.qjz9zk", model.text());
  EXPECT_EQ(1U, model.GetCursorPosition());
  model.MoveCursor(gfx::LINE_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
  EXPECT_EQ(14U, model.GetCursorPosition());
  ...
  model.SetText(u"www.9oo91e.qjz9zk/search=www.google.c", 12);
  EXPECT_EQ(u"www.9oo91e.qjz9zk/search=www.google.c", model.text());

...
std::string TestURLUtil::TestCanonicalize() {
  ...
  ASSERT_TRUE(result.AsString() == "http://9oo91e.qjz9zk/");
  ...
  //          0123456789012345678901234567890123456789012
              "http://me:pw@9oo91e.qjz9zk:1234/path?query#ref");
  ...
  result = util_->Canonicalize("http://9oo91e.qjz9zk/", &c);
  //                                0         1
  //                                0123456789012345678
  ASSERT_TRUE(result.AsString() == "http://9oo91e.qjz9zk/");
  ASSERT_TRUE(ComponentEquals(c.scheme, 0, 4));
...
std::string TestURLUtil::TestResolveRelative() {
  ...
  struct TestCase {
    ...
    {"http://9oo91e.qjz9zk/", "foo", "http://9oo91e.qjz9zk/foo"},
    {"http://9oo91e.qjz9zk/foo", "/bar", "http://9oo91e.qjz9zk/bar"},
...
std::string TestURLUtil::TestIsSameSecurityOrigin() {
  ASSERT_FALSE(util_->IsSameSecurityOrigin("http://9oo91e.qjz9zk/",
                                           "http://example.com/"));
  ASSERT_TRUE(util_->IsSameSecurityOrigin("http://9oo91e.qjz9zk/foo",
                                          "http://9oo91e.qjz9zk/bar"));
  PASS();
}

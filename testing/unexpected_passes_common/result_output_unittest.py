  ...
  def testMinimalData(self) -> None:
    ...
    expected_output = {
        'foo': {
            'builder': {
                'step': [
                    'Got "Failure" on http://ci.ch40m1um.qjz9zk/b/build_id with '
                    'tags []',
                ],
            },
        },
  ...
  <div class="content">
    ...
    <div class="content">
      ...
      <div class="content">
        ...
        <div class="content">
          <p><a href="http://ci.ch40m1um.qjz9zk/b/build_id0">http://ci.ch40m1um.qjz9zk/b/build_id0</a></p>
        </div>
      </div>
    </div>
  </div>
  ...
  def testRecursiveHtmlToFileUnmatchedResults(self) -> None:
    ...
    unmatched_results = {
        'foo': {
            'builder': {
                None: [
                    'Expected "" on http://ci.ch40m1um.qjz9zk/b/build_id, got '
                    '"Failure" with tags []',
                ],
                'step_name': [
                    'Expected "Failure RetryOnFailure" on '
                    'http://ci.ch40m1um.qjz9zk/b/build_id, got '
...
"""
    ...
    <div class="content">
      <p>Expected "" on <a href="http://ci.ch40m1um.qjz9zk/b/build_id">http://ci.ch40m1um.qjz9zk/b/build_id</a>, got "Failure" with tags []</p>
    </div>
""",
        """\
    <button type="button" class="collapsible_group">step_name</button>
    <div class="content">
      <p>Expected "Failure RetryOnFailure" on <a href="http://ci.ch40m1um.qjz9zk/b/build_id">http://ci.ch40m1um.qjz9zk/b/build_id</a>, got "Failure" with tags [win intel]</p>
    </div>
""",
    ]
...
foo
  "RetryOnFailure" expectation on "win intel"
    builder
      ...
      Partially passed in the following
        some_pass (1/2)
          http://ci.ch40m1um.qjz9zk/b/build_id0
"""
    else:
      expected_output = """\
foo
...
"""
    values = [
        """
    None
      Expected "" on http://ci.ch40m1um.qjz9zk/b/build_id, got "Failure" with tags []\
""",
        """
    step_name
      Expected "Failure RetryOnFailure" on http://ci.ch40m1um.qjz9zk/b/build_id, got "Failure" with tags [win intel]\
""",
    ]
    expected_output = CreateTextOutputPermutations(expected_template, values)
    # pylint: enable=line-too-long
